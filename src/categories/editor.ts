...
export function categoryState(definitions:CategoryDefinition[],id:string):{path:string;enabled:boolean;audience:CategoryDefinition['audience']} {
 ...
 while(current!==null){const c=byId.get(current);if(!c||visited.has(current)||visited.size>=10)return {path:names.reverse().join(' / ')||'分类暂时不可用',enabled:false,audience:'admin'};
  visited.add(current);names.push(c.name);enabled=enabled&&c.enabled;if(rank[c.audience]>rank[audience])audience=c.audience;current=c.parentId;
 }
