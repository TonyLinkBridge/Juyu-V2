...
export function printEditor(nodes:EditorBlock[],media:(b:MediaBlock)=>string):string{
 const render=(nodes:EditorBlock[]):string=>{
  let html='';for(let i=0;i<nodes.length;i++){const b=nodes[i];
   ...
   const tag=b.type==='heading'?`h${Math.min((b.props.level??1)+1,6)}`:b.type==='quote'?'blockquote':'p';html+=`<${tag}${b.type==='heading'?` id="${b.id}"`:''} style="${css(b.props)}">${b.type==='checkListItem'?(b.props.checked?'☑ ':'☐ '):''}${marks(b.content)}</${tag}>${b.children.length?`<div style="padding-left:6mm">${render(b.children)}</div>`:''}`;
  }return html;
 };return render(nodes);
}
