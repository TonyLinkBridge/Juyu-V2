...
export function writeLocalCopy(store:Storage,copy:LocalCopy){if(!copy.owner||copy.text.length>2_000_000)throw Error('RECOVERY_LIMIT');store.setItem(key(copy.owner,copy.id),JSON.stringify(copy));if(copy.sequence===null){const value=JSON.parse(copy.text);if((value.locale===undefined||value.locale==='zh-CN')&&['article','ops','qa','reference'].includes(value.kind))store.setItem(key(copy.owner,'new-'+value.kind),copy.id);}}
