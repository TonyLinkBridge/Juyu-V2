...
import {invalid as bad,record,keys,boundedText,color,normalizeInline,type EditorInline} from './inline.ts';
...
export function normalizeEditorBlocks(value:unknown):EditorBlock[]{
 ...
 const visit=(input:unknown,depth:number):EditorBlock=>{
  ...
  if(b.type==='juyu'){keys(p,['payload']);if(b.content!==undefined||typeof p.payload!=='string'||p.payload.length>250000)return bad();let m:MediaBlock;try{m=normalizeBlocks([JSON.parse(p.payload)])[0];}catch{return bad();}if(m.id!==id)return bad();media.push(m);return {id,type:'juyu',props:{payload:JSON.stringify(m)},children:Array.from(child,c=>visit(c,depth+1))};}
...
export function editorMedia(blocks:EditorBlock[]):MediaBlock[]{const result:MediaBlock[]=[];const visit=(nodes:EditorBlock[])=>{for(const b of nodes){if(b.type==='juyu')result.push(JSON.parse(b.props.payload));else if(isFileBlock(b)&&b.props.url)result.push({id:b.id,type:b.type,assetId:privateAssetId(b.props.url)!,caption:b.props.caption,alt:b.props.name});visit(b.children);}};visit(normalizeEditorBlocks(blocks));return normalizeBlocks(result);}
