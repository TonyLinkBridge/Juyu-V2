...
export default async function FormsSettingsPage(){const access=await currentAdminAccess();if(access.status!=='admin')redirect(adminDestination(access));let initial:FormDefinition[]|undefined,definitions:FieldDefinition[]|undefined,state:'ready'|'denied'|'unavailable'='unavailable';try{const service=await applicationAuthorization();[initial,definitions]=await Promise.all([service.forms(true),service.fields()]);state='ready';}catch(e){if(e instanceof Error&&e.message.startsWith('FORBIDDEN'))state='denied';}return <FeaturePage feature="forms" admin><EntryShell><main id="main-content"><FormSettings initial={initial} definitions={definitions} state={state}/><a className="secondary-link" href="/admin/settings/history">设置变更记录</a><EmployeeSignOut audience="admin"/></main></EntryShell></FeaturePage>;}
