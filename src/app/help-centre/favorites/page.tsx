...
export default async function FavoritesCollectionPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <FeaturePage feature="favorites" locale={locale}><EntryShell navigation={<ReaderMenu currentHref="/help-centre/favorites"/>} search={locale==='en'?<SearchInput locale="en"/>:<FeatureSearch/>}><main id="main-content" className="editor-main search-main"><FavoritesView viewerId={access.userId} data={data} state={state} locale={locale}/><div className="reader-actions"><EmployeeSignOut/></div></main></EntryShell></FeaturePage>;
}
