...
export default async function QaCollectionPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <EntryShell account navigation={<ReaderMenu currentHref="/help-centre/qa"/>} search={locale==='en'?<SearchInput locale="en"/>:<FeatureSearch/>}><main id="main-content" className="editor-main search-main"><QaView searchEnabled={searchEnabled} data={data} state={state} viewerId={access.userId} initialAnswer={state==='ready'?initialAnswer:undefined} locale={locale}/><div className="reader-actions"><EmployeeSignOut/></div></main></EntryShell>;
}
