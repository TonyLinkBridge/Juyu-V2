...
export default async function RecentCollectionPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <FeaturePage feature="recent" locale={locale}><EntryShell navigation={<ReaderMenu currentHref="/help-centre/recent"/>} search={locale==='en'?<SearchInput locale="en"/>:<FeatureSearch/>}><main id="main-content" className="editor-main search-main"><RecentView data={data} state={state} locale={locale}/><div className="reader-actions"><EmployeeSignOut/></div></main></EntryShell></FeaturePage>;
}
