...
export async function GET(request:Request,context:{params:Promise<{id:string}>}){return availabilityResponse(async()=>{
 ...
 return service.availabilityDetail((await context.params).id);
});}
export async function POST(request:Request,context:{params:Promise<{id:string}>}){return availabilityResponse(async()=>{
 ...
 return service.changeAvailability((await context.params).id,input);
});}
