...
export default async function DesignPreview({params}:{params:Promise<{screen:string}>}){
 ...
 if(screen==='article-email'){
  ...
  return <>{notice}<EntryShell search={<SearchInput/>}><ReaderNavigation pages={emailPages} requested="email-preview" article={{id:'email-preview',title:'如何修改账户邮箱',revision:22,body:emailBody}} articleActions={<button type="button" className="secondary-link" disabled aria-label="收藏此文章（仅布局示例）">收藏此文章</button>}/></EntryShell></>;
 }
