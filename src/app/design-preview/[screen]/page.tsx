...
import {PreviewArticleActions} from '../../../components/preview-article-actions';
...
export default async function DesignPreview({params}:{params:Promise<{screen:string}>}){
 ...
 if(screen==='article-email'){
  ...
  return <>{notice}<EntryShell search={<SearchInput/>}><ReaderNavigation pages={emailPages} requested="email-preview" article={{id:'email-preview',title:'如何修改账户邮箱',revision:22,publicationNumber:1,feedback:{memberId:'preview',value:null},body:emailBody}} articleActions={<PreviewArticleActions/>}/></EntryShell></>;
 }
