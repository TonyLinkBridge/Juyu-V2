...
export interface DecisionInput {expectedSequence:number;action:'approve'|'reject';reason?:string;englishReviewConfirmed?:true}
...
export function decisionInput(value:unknown):DecisionInput {
 ...
 if(Object.keys(x).some(k=>!['expectedSequence','action','reason','englishReviewConfirmed'].includes(k))||!Object.hasOwn(x,'expectedSequence')||!Object.hasOwn(x,'action')
 ...
 if(Object.hasOwn(x,'englishReviewConfirmed')&&x.englishReviewConfirmed!==true)throw new Error('INVALID_INPUT');
 const reason=typeof x.reason==='string'?x.reason.trim():'';
 if(reason.length>2000||reason.includes('\0')||(x.action==='approve'&&reason)||(x.action==='reject'&&x.englishReviewConfirmed))throw new Error('INVALID_INPUT');
 if(x.action==='reject'&&!reason)throw new Error('REASON_REQUIRED');
 return {expectedSequence:Number(x.expectedSequence),action:x.action as DecisionInput['action'],...(x.action==='reject'?{reason}:{}),...(x.englishReviewConfirmed===true?{englishReviewConfirmed:true as const}:{})};
}
