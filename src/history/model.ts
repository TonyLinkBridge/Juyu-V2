...
export interface HistoryVersion {documentId:string;sequence:number;currentRevision:number;publishedRevision:number|null;lifecycle:'active'|'archived'|'trashed';status:Status;version:VersionSummary & {customFields?:FieldSnapshot[];qa?:QaMetadata;body:string;audience:Audience;tags:string[];cover:ArticleCover|null;blocks:MediaBlock[]};categories:{id:string;name:string}[];assets:{id:string;filename:string;mime:string;size:string;status:string}[];canRestore:boolean}
