CREATE FUNCTION juyu.search_publications_scoped(terms text[],requested_page integer,requested_kind text)
...
BEGIN
 ...
 IF requested_kind NOT IN ('article','ops','reference','qa') OR requested_kind IS NULL
  OR requested_page IS NULL OR requested_page<1 OR requested_page>999999 OR terms IS NULL OR cardinality(terms)<1
 ...
 WITH matches AS MATERIALIZED (
  ...
  WHERE d.kind=requested_kind AND s.grams @> query_grams AND juyu.can_read_revision(s.document_id,s.revision_id)
...
END $$;
REVOKE ALL ON FUNCTION juyu.search_publications_scoped(text[],integer,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION juyu.search_publications_scoped(text[],integer,text) TO juyu_runtime;
