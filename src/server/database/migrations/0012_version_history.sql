-- Preserve earlier migrations and rows; restoration records the immutable source.
ALTER TABLE juyu.audit_log ADD COLUMN source_revision_id integer;
ALTER TABLE juyu.audit_log ADD CONSTRAINT audit_source_revision_fk FOREIGN KEY(document_id,source_revision_id) REFERENCES juyu.revisions(document_id,revision_id) DEFERRABLE INITIALLY DEFERRED;
ALTER TABLE juyu.audit_log DROP CONSTRAINT audit_log_action_check;
ALTER TABLE juyu.audit_log ADD CONSTRAINT audit_log_action_check CHECK(action IN('create','edit','submit','withdraw','reassign','reject','approve','queue','publish','trash','restore','archive','unpublish','unarchive','restore_version'));
ALTER TABLE juyu.audit_log ADD CONSTRAINT audit_restore_source_check CHECK((action='restore_version')=(source_revision_id IS NOT NULL) AND (source_revision_id IS NULL OR source_revision_id<revision_id));
CREATE FUNCTION juyu.restore_document_version(p_document text,p_source integer,p_expected integer)
...
BEGIN
 ...
 INSERT INTO juyu.revisions(document_id,revision_id,title,body,audience,author_id,editor_id,created_at,tags,cover_alt,cover_position,content_blocks)
 VALUES(p_document,next_revision,source.title,source.body,source.audience,source.author_id,who,stamp,source.tags,source.cover_alt,source.cover_position,source.content_blocks);
