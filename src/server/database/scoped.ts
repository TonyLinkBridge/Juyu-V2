...
export class ScopedDatabase implements Transactions {
  ...
  async run<T>(viewer: Viewer | null, work: (client: PoolClient) => Promise<T>, readOnly = false): Promise<T> {
    ...
    try {
      ...
      issuer = await this.issuer.connect();
      await checkRole(issuer, 'juyu_context_issuer', 'juyu_runtime');
      const token = randomBytes(32).toString('hex');
      hash = createHash('sha256').update(token).digest('hex');
      await issuer.query('DELETE FROM juyu.request_contexts WHERE expires_at<clock_timestamp()');
      // The context must exist before a repeatable-read transaction takes its first snapshot.
      await issuer.query("INSERT INTO juyu.request_contexts(token_hash,backend_pid,member_id,role,expires_at) VALUES ($1,$2,$3,$4,clock_timestamp()+interval '60 seconds')", [hash, pid, viewer.id, viewer.role]);
