...
export async function exportPDF(request:Request,id:string,revision:number,deps:Dependencies):Promise<Response>{
 try{
  ...
  const snapshot=await deps.snapshot(id,revision);if(snapshot.article.id!==id||snapshot.article.revision!==revision)throw new Error('VERSION_CHANGED');
  ...
  for(const assetId of assetIds){const asset=await deps.asset(assetId);if(!asset||asset.id!==assetId||asset.document_id!==id||(imageIds.has(assetId)&&!['image/png','image/jpeg','image/webp','image/gif'].includes(asset.mime_type)))throw new Error('IMAGE_UNAVAILABLE');validateAsset(asset);assets.push(asset);}
  ...
  for(const asset of assets.filter(a=>imageIds.has(a.id))){
   ...
   const signal=AbortSignal.any([request.signal,AbortSignal.timeout(15000)]);const response=await deps.storage().read(asset.object_key,undefined,signal);
   ...
   let bytes:Buffer;try{bytes=await readBounded(response.body,size,signal);}catch{throw new Error('IMAGE_UNAVAILABLE');}if(bytes.length!==size)throw new Error('IMAGE_UNAVAILABLE');images[asset.id]=`data:${asset.mime_type};base64,${bytes.toString('base64')}`;
  }
  ...
  const bytes=await deps.render(pdfHTML(snapshot.article,cover,images));request.signal.throwIfAborted();
  // Generation can take seconds. Re-read current identity, publication and assets before release.
  const current=await deps.snapshot(id,revision);if(JSON.stringify(current)!==JSON.stringify(snapshot))throw new Error('VERSION_CHANGED');
  for(const asset of assets)if(JSON.stringify(await deps.asset(asset.id))!==JSON.stringify(asset))throw new Error('IMAGE_UNAVAILABLE');
