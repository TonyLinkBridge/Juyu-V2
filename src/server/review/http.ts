...
export async function reviewResponse(action:()=>Promise<unknown>):Promise<Response>{
 ...
 try{return Response.json(await action(),{headers});}
  ...
  const status=['FORBIDDEN','NOT_REVIEWER'].includes(code)?403:code==='NOT_FOUND'?404:['EDIT_REQUIRED','INVALID_APPROVAL','INVALID_MEDIA','CONFLICT','INVALID_REVIEWER','UPLOAD_IN_PROGRESS','INVALID_STATE','INACTIVE_DOCUMENT'].includes(code)?409:['INVALID_INPUT','REASON_REQUIRED','ENGLISH_REVIEW_REQUIRED'].includes(code)?400:code==='UPLOAD_TOO_LARGE'?413:503;
