export async function editorResponse(action:()=>Promise<unknown>):Promise<Response> {
 ...
 try{return Response.json(await action(),{headers});}
  ...
  const code=['FORBIDDEN','AUTH_NOT_CONFIGURED','NOT_FOUND','CONFLICT','FIELD_CONFLICT','INVALID_STATE','INACTIVE_DOCUMENT','INVALID_INPUT','INVALID_MEDIA','INVALID_COVER','INVALID_PRESENTATION','USE_EDITOR','UPLOAD_TOO_LARGE'].includes(raw)?raw:'EDITOR_UNAVAILABLE';
