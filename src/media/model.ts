...
export type MediaBlock=ScienceBlock|TextBlock|{id:string;type:'image'|'video'|'audio'|'file';assetId:string;caption:string;alt:string}|{id:string;type:'table';headers:string[];rows:string[][]};
...
export function normalizeBlocks(value:unknown):MediaBlock[]{
 ...
 const result=value.map(b=>{
  if(!b||typeof b!=='object'||!['image','video','audio','file','table','hint','code','tabs','math','diagram'].includes(b.type)||typeof b.id!=='string'||!/^[-a-zA-Z0-9]{1,64}$/.test(b.id)||ids.has(b.id))return bad();ids.add(b.id);
  ...
  return {id:b.id,type:b.type as 'image'|'video'|'audio'|'file',assetId:b.assetId,caption:text(b.caption,500),alt:text(b.alt,500)};
