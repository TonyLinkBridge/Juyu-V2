...
export function normalizeBlocks(value:unknown):MediaBlock[]{
 ...
 const result=value.map(b=>{
  ...
  if(b.type==='hint'){
   if(Object.keys(b).some(k=>!['id','type','style','title','body','iconKey'].includes(k))||!['info','success','warning','danger'].includes(b.style)||b.iconKey!==undefined&&b.iconKey!==null&&readerIconKey(b.iconKey)===null)return bad();
   return {id:b.id,type:'hint' as const,style:b.style as 'info'|'success'|'warning'|'danger',title:text(b.title,200),body:text(b.body,20000),...(b.iconKey===undefined?{}:{iconKey:b.iconKey as ReaderIconKey|null})};
  }
  if(b.type==='code'){
