 ...
 const render=(nodes:EditorBlock[]):ReactNode[]=>{
  ...
  for(let i=0;i<nodes.length;i++){
   ...
   if(b.type==='table'){
    ...
    result.push(<Fragment key={b.id}><div className="reader-scroll-region" tabIndex={0} role="region" aria-label={locale==='en'?'Table; scroll horizontally to view more columns':'资料表格，可横向滚动'}><table style={style(b.props)}><colgroup>{c.columnWidths.map((w,j)=><col key={j} style={{width:w}}/>)}</colgroup><tbody>{c.rows.map((row,r)=>{let col=0;return <tr key={r}>{row.cells.map((cell,j)=>{while(occupied[r][col])col++;const header=r<(c.headerRows??0)||col<(c.headerCols??0),Tag=header?'th':'td';for(let y=r;y<r+(cell.props.rowspan??1);y++)for(let x=col;x<col+(cell.props.colspan??1);x++)occupied[y][x]=true;col+=cell.props.colspan??1;return <Tag key={j} colSpan={cell.props.colspan} rowSpan={cell.props.rowspan} style={style(cell.props)}><StructuredInline content={cell.content} documentId={documentId} revision={revision} admin={admin} locale={locale}/></Tag>;})}</tr>;})}</tbody></table></div>{children}</Fragment>);continue;
   }
