...
export function PageAside({sections,article,articleActions,features}:{sections:DocumentSection[];article:Publication;articleActions?:ReactNode;features?:FeatureFlags}) {
 return <aside className="gitbook-page-aside group/aside" aria-label="文章目录与操作">
   ...
   {features?.feedback!==false&&<PageFeedbackForm key={`${article.id}:${article.revision}:${article.feedback?.memberId??'current'}`} documentId={article.id} revision={article.revision} initial={article.feedback?.value} publicationNumber={article.publicationNumber}/>}
 </aside>;
}
