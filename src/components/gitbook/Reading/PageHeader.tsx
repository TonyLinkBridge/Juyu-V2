...
export function PageHeader({section,article,navigation}:{section?:'ops';article:Publication;navigation:PageNavigation}) {
 ...
 return <header className="gitbook-page-header">
   ...
   {article.englishId&&<nav className="reader-language-switch" aria-label={english?'Article language':'文章语言'}><a href={`/help-centre?article=${encodeURIComponent(article.sourceId??article.id)}`} aria-current={!english?'page':undefined}>简体中文</a><a href={`/help-centre?article=${encodeURIComponent(article.englishId)}&lang=en`} aria-current={english?'page':undefined}>English</a></nav>}
