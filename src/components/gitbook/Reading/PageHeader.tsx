...
export function PageHeader({section,article,navigation}:{section?:'ops';article:Publication;navigation:PageNavigation}) {
 return <header className="gitbook-page-header">
   <nav aria-label="面包屑" className="reader-breadcrumbs"><ol>
     <li><Link prefetch={false} href={section==='ops'?'/help-centre/ops':'/help-centre/library'}>{section==='ops'?'OPS Internal':'资料目录'}</Link>{navigation.ancestors.length>0&&<BreadcrumbSeparator/>}</li>
     {navigation.ancestors.map((ancestor,index)=><li key={ancestor.id}><span>{ancestor.title}</span>{index<navigation.ancestors.length-1&&<BreadcrumbSeparator/>}</li>)}

   </ol></nav>
   <p className="reader-eyebrow article-publication-status"><span aria-hidden="true"/> 已发布 · 正式资料</p>
   <PageTags tags={article.tags}/>
   <h1 id="reader-page-title" tabIndex={-1}><FileText className="reader-title-icon" size={32} aria-hidden="true"/><span>{article.title}</span></h1><p className="reader-version">{article.publicationNumber?`正式版本 ${article.publicationNumber}`:'已发布'}</p>
 </header>;
}
