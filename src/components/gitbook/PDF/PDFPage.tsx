...
export function PDFPage({snapshot}:{snapshot:PDFSnapshot}){
 ...
 return <main id="main-content" className="pdf-shell"><style>{pdfCSS}</style><PDFPrintControls publicationNumber={article.publicationNumber} key={`${article.id}:${article.revision}`} documentId={article.id} revision={article.revision} coverId={article.cover?.assetId??null}/>
