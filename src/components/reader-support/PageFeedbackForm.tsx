...
export function PageFeedbackForm({documentId,revision,initial,publicationNumber,locale='zh-CN'}:{publicationNumber?:number|null;documentId:string;revision:number;initial?:SavedFeedback|null;locale?:'zh-CN'|'en'}){
 ...
 const failLoad=useCallback(()=>{setError(label('反馈暂时无法读取，你仍可继续阅读文章。','Feedback is unavailable, but you can keep reading.'));setBusy(false);},[setError,setBusy,label]);
 ...
 async function submit(event:React.FormEvent){
  ...
  try{
   ...
   if(!response.ok){
    if(result.error==='CONFLICT'){setBlocked(true);setError(label('这份反馈已在另一页面更新。请重新读取后再修改；重新读取会替换当前输入。','Your feedback changed in another tab. Reload it before editing; this will replace your current input.'));return;}
    if(result.error==='VERSION_CHANGED'||response.status===404||response.status===403){setBlocked(true);setError(label('文章版本或访问权限已变化，请刷新文章后再反馈。','This article or your access has changed. Refresh the article before sending feedback.'));return;}
    setError(label('反馈未确认保存，输入已保留。请重试。','We couldn’t confirm your feedback was saved. Your input is still here; please try again.'));return;
   }
   setSaved(result.feedback);setHelpful(result.feedback.helpful);setComment(result.feedback.comment??'');setSuccess(true);
  }catch{if(request.current===controller)setError(label('反馈未确认保存，输入已保留。请重试。','We couldn’t confirm your feedback was saved. Your input is still here; please try again.'));}
  finally{clearTimeout(timer);if(request.current===controller)setBusy(false);}
 }
 const edited=()=>{setSuccess(false);setError('');};
 return <section aria-label={label('文章反馈','Article feedback')} className="reader-feedback">
  <form onSubmit={submit}>
   <div className="feedback-rating"><p id={`${id}-question`}>{label('这篇文章有帮助吗？','Was this article helpful?')}</p><div role="group" aria-labelledby={`${id}-question`}>
    {([true,false] as const).map(value=><button key={String(value)} type="button" aria-pressed={helpful===value} disabled={!loaded||busy||blocked} onClick={()=>{setHelpful(value);edited();requestAnimationFrame(()=>commentRef.current?.focus());}}>{value?<ThumbsUp size={17} aria-hidden="true"/>:<ThumbsDown size={17} aria-hidden="true"/>}<span className="feedback-label">{value?label('有帮助','Yes'):label('没有帮助','No')}</span></button>)}
   </div></div>
   <p className="feedback-privacy">{locale==='en'?`Your feedback is for ${publicationNumber?`published version ${publicationNumber}`:'the current published version'}. Admins can see your rating and note.`:`反馈对应${publicationNumber?`正式版本 ${publicationNumber}`:'当前已发布内容'}，管理员可查看你的选择和说明。`}</p>
   {helpful!==null&&<div className="feedback-comment"><label htmlFor={`${id}-comment`}>{label('补充说明（选填）','Add a note (optional)')}</label><textarea ref={commentRef} id={`${id}-comment`} rows={3} maxLength={1000} value={comment} disabled={busy||blocked} onChange={e=>{setComment(e.target.value);edited();}}/>
    <div className="feedback-submit"><span>{comment.length}/1000</span><button className="secondary-link" type="submit" disabled={busy||blocked||!loaded}>{busy?label('正在保存…','Saving…'):saved?label('更新反馈','Update feedback'):label('提交反馈','Send feedback')}</button></div>
   </div>}
   <p role="status" className="feedback-status">{success?label('反馈已保存，谢谢。你可以继续修改。','Thanks, your feedback is saved. You can still edit it.'):!loaded&&busy?label('正在读取反馈…','Loading feedback…'):saved&&!error?label('已载入你之前的反馈，可修改后保存。','Your previous feedback is ready to edit.'):''}</p>
   {error&&<p role="alert" className="feedback-error">{error}</p>}
   {((!loaded&&!busy)||blocked)&&<button type="button" className="secondary-link" disabled={busy} onClick={()=>{setBusy(true);setSuccess(false);setError('');setLoaded(false);void load();}}>{label('重新读取反馈','Reload feedback')}</button>}
  </form>
 </section>;
}
