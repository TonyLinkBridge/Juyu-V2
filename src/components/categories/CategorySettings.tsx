...
export function CategorySettings({initial=[],state='ready'}:{initial?:CategoryDefinition[];state?:'ready'|'unavailable'|'denied'}) {
  ...
  function categoryBranch(category:CategoryDefinition,depth:number):ReactNode {
    ...
    return <li key={category.id}><button type="button" className="category-settings-item" disabled={frozen} aria-pressed={draft?.id===category.id} onClick={()=>select(category)}><span className="category-tree-title"><span aria-hidden="true">{depth===0?'▱':'↳'}</span><strong>{category.name}</strong></span><span className="category-tree-meta">{depth===0?'主分类':'子分类'} · {category.enabled?'已启用':'已停用'}</span><small>实际访问：{effectivePolicy(categories,category)}</small></button>{children.length>0&&<ul aria-label={`${category.name}的子分类`}>{children.map(row=>categoryBranch(row.category,row.depth))}</ul>}</li>;
  }
