...
export function AnalyticsDashboard({data,state='ready',days=30}:{data?:DashboardData;state?:State;days?:7|30|90}){
 ...
 <div className="analytics-grid">
  ...
  <section className="analytics-section" aria-label="热门资料"><h2>热门资料</h2><p>当前可读的正式资料，按打开次数排序，最多前 10 篇。显示最新正式标题，次数包含其旧版本访问。</p>{data.popularArticles.length?<ol className="analytics-ranking">{data.popularArticles.map(item=><li key={item.documentId}><div><a href={contentPath(item.kind,item.documentId)}>{item.title}</a><p>{kinds[item.kind]} · 当前内部修订 {item.revision}</p></div><span><strong>{item.views}</strong> 次打开</span></li>)}</ol>:<p className="analytics-empty">所选时间内没有当前可读资料的打开记录。</p>}</section>
  <section className="analytics-section" aria-label="需要改进的资料"><h2>需要改进的资料</h2><p>当前正式版本的有效评价，按“没有帮助”数量排序，最多前 10 篇。仅统计最后修改时间在所选范围内的评价，每员工每版本一份。</p><p className="analytics-feedback-summary">共 {data.summary.feedbackTotal} 份评价 · {data.summary.feedbackNegative} 份没有帮助 · 占比 {analyticsPercent(data.summary.feedbackNegative,data.summary.feedbackTotal)}</p>{data.negativeFeedback.length?<ol className="analytics-ranking">{data.negativeFeedback.map(item=><li key={item.documentId}><div><a href={`/admin/feedback?document=${encodeURIComponent(item.documentId)}&revision=${item.revision}`}>{item.title}</a><p>{kinds[item.kind]} · 当前内部修订 {item.revision}</p></div><span><strong>{item.negative} / {item.total}</strong> 份没有帮助</span></li>)}</ol>:<p className="analytics-empty">所选时间内没有当前正式版本的负面评价。</p>}<a className="back-link" href="/admin/feedback">查看文章反馈 →</a></section>
 </div>
 </>}
 </main>;
}
