...
export function FeedbackDashboard({overview,details}:{overview?:FeedbackOverview;details?:FeedbackDetails}){
 if(details){const {summary:s}=details;return <>
  <a className="back-link" href="/admin/feedback">← 全部文章反馈</a><h2>{s.title}</h2><p className="access-description">内部修订 {s.revision} · {s.current?'当前已发布版本':'历史版本或已下线'} · 共 {s.total} 份反馈</p>
