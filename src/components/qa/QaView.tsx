...
export function QaView({data,state,initialAnswer,viewerId,searchEnabled=true,locale='zh-CN'}:{searchEnabled?:boolean;initialAnswer?:Publication;viewerId?:string;data?:QaPage;state:'ready'|'denied'|'unavailable'|'invalid';locale?:'zh-CN'|'en'}){
 ...
 {searchEnabled&&<Form action="/help-centre/qa" className="qa-search"><label className="sr-only" htmlFor="qa-search">{label('搜索问题或答案','Search questions or answers')}</label><input key={data.q??''} id="qa-search" name="q" type="search" maxLength={120} defaultValue={data.q??''} placeholder={label('例如：转入失败、修改邮箱、退款','For example: transfer failed, change email, refund')}/>{data.category!==undefined&&<input type="hidden" name="category" value={data.category}/>}{english&&<input type="hidden" name="lang" value="en"/>}<button type="submit">{label('搜索问答','Search Q&A')}</button></Form>}
 <nav className="qa-categories" aria-label={label('问答分类','Q&A categories')}><Link prefetch={false} href={href(data,1,undefined,locale)} aria-current={data.category===undefined?'page':undefined}>{label('全部','All')}</Link>{(data.categories??[]).map(category=><Link prefetch={false} key={category} href={href(data,1,category,locale)} aria-current={data.category===category?'page':undefined}>{category||label('未分类','Uncategorized')}</Link>)}</nav>
