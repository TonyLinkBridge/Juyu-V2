...
export function QaView({data,state}:{data?:QaPage;state:'ready'|'denied'|'unavailable'}){
 ...
 {data.items.length?<div className="qa-question-list">{data.items.map(item=><QaAnswer key={item.id} id={item.id} title={item.title} revision={item.revision} canEdit={data.canEdit}/>)}</div>:<div className="qa-empty"><h2>暂时没有找到相关答案</h2><p>试试其他关键词，或查看全部问答。</p><Link href="/help-centre/qa">查看全部问答</Link></div>}
