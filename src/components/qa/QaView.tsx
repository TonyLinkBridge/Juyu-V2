...
import {QaEditorialIndex} from './QaEditorialIndex';
...
export function QaView({data,state,initialAnswer,previewAnswers,viewerId,searchEnabled=true,locale='zh-CN',withinDocsPage=false}:{searchEnabled?:boolean;initialAnswer?:Publication;previewAnswers?:Record<string,Publication>;viewerId?:string;data?:QaPage;state:'ready'|'denied'|'unavailable'|'invalid';locale?:'zh-CN'|'en';withinDocsPage?:boolean}){
 const english=locale==='en',label=(zh:string,en:string)=>english?en:zh;
 const root=english?'/help-centre/qa?lang=en':'/help-centre/qa';
 return <section className="qa-view qa-independent" aria-label={label('Q&A 正式问答','Published Q&A')}>{!withinDocsPage&&<header><p className="reader-eyebrow">Q&A</p><h1>{label('你遇到了什么问题？','What do you need to know?')}</h1><p className="reader-description">{label('查找问题，展开查看已审核发布的标准答案。','Find a question and open its reviewed, published answer.')}</p></header>}{state!=='ready'||!data?<div role="alert">{!withinDocsPage&&<h2>{state==='denied'?label('问答阅读权限不足','You cannot access these answers'):state==='invalid'?label('问答筛选条件无效','Invalid Q&A filter'):label('问答服务暂时不可用','Q&A is unavailable')}</h2>}<p>{state==='denied'?label('请确认公司账号，或联系管理员核对阅读权限。','Check your company account or contact an administrator.'):state==='invalid'?label('请清除筛选后重新查看问答。','Clear the filter and try again.'):label('请稍后重新读取；如果持续失败，请联系管理员。','Try again shortly. If the problem continues, contact an administrator.')}</p><Link href={root}>{label('重新读取','Try again')}</Link></div>:<>
 ...
 {data.items.length?<QaEditorialIndex items={data.items} viewerId={viewerId} initialAnswer={initialAnswer} previewAnswers={previewAnswers} canEdit={data.canEdit} locale={locale}/>:<div className="qa-empty"><h2>{label('暂时没有找到相关答案','No matching answers yet')}</h2><p>{label('试试其他关键词，或查看全部问答。','Try different keywords or browse all questions.')}</p><Link href={root}>{label('查看全部问答','View all Q&A')}</Link></div>}
