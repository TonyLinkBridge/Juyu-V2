...
export function RecentView({data,state}:{data?:RecentPage;state:'ready'|'denied'|'unavailable'}){
 ...
 {data.items.length?<ul className="gitbook-search-results" aria-label="最近打开的正式资料">{data.items.map(item=><li key={item.id}><SearchResultItem href={contentPath(item.kind,item.id)} label={`再次阅读：${item.title}`} leadingIcon={<span aria-hidden="true">↺</span>}><p className="search-result-breadcrumbs">{kinds[item.kind]} · 已发布{item.viewedRevision!==item.revision?' · 上次阅读后有更新':''}</p><h2>{item.title}</h2>{item.tags.length>0&&<p className="search-result-breadcrumbs">{item.tags.join(' · ')}</p>}<p className="search-result-breadcrumbs">上次打开 <time dateTime={item.viewedAt}>{viewedTime(item.viewedAt)}</time>（UTC+8）</p></SearchResultItem></li>)}</ul>:<div className="search-state"><h2>暂无可阅读的浏览记录</h2><p>打开正式文章后会自动记录。已下线或不再有权阅读的资料不会显示。</p><a className="secondary-link" href="/help-centre">前往资料库</a></div>}
