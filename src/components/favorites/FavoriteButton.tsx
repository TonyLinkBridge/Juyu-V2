...
function FavoriteControl({kind='article',documentId,revision,initial,onChange,label='文章收藏'}:Props){
 ...
 useEffect(()=>{
  ...
  const focus=()=>{if(!onChange)void load();};window.addEventListener('focus',focus);
  return()=>{sequence.value++;operation.current=false;window.removeEventListener('focus',focus);};
 },[hasInitial,load,reload,onChange]);
 async function change(){
  ...
  try{const result=await setFavoriteState(documentId,revision,desired.saved);if(token!==sequence.value)return;pendingRequest.current=null;setPending(null);setSaved(result.saved);setNotice(result.saved?'已加入我的收藏。':'已取消收藏。');onChange?.(result);}
