...
type Props={data?:FavoritesPage;state:'ready'|'denied'|'unavailable'};
export function FavoritesView(props:Props){return <FavoritesCollection key={JSON.stringify(props)} {...props}/>;}
function FavoritesCollection({data:initial,state}:Props){
 ...
 {data.items.length?<ul className="gitbook-search-results favorites-results" aria-label="已收藏的正式资料">{data.items.map(item=><li key={item.id}><SearchResultItem href={contentPath(item.kind,item.id)} label={`阅读收藏：${item.title}`} leadingIcon={<span aria-hidden="true">☆</span>}><p className="search-result-breadcrumbs">{kinds[item.kind]} · 正式版本 {item.revision}</p><h2>{item.title}</h2>{item.tags.length>0&&<p className="search-result-breadcrumbs">{item.tags.join(' · ')}</p>}</SearchResultItem><div className="favorite-item-actions"><FavoriteButton documentId={item.id} revision={item.revision} initial={{documentId:item.id,revision:item.revision,saved:true}} onChange={update} kind={item.kind} label={`收藏操作：${item.title}`}/></div></li>)}</ul>:<div className="search-state"><h2>{data.total===0?'暂无可阅读的收藏':'本页收藏已取消'}</h2><p>打开一篇正式文章，点击“收藏文章”，即可在这里找到。有些已收藏资料可能已下线或不再有访问权限。</p><a className="secondary-link" href="/help-centre">前往资料库</a></div>}
