...
function cells(items:VisibleReferenceCell[]){
 return items.map(({cell,column,rowSpan,colSpan,header})=>{
  const Tag=header?'th':'td';
  return <Tag key={column} rowSpan={rowSpan} colSpan={colSpan} style={structuredStyle(cell.props)}><StructuredInline content={cell.content}/></Tag>;
 });
}
export function NativeReferenceTable({table,indices,number}:{table:ReferenceTableData;indices:number[];number:number}){
 ...
 return <table className="reference-rich-table" style={structuredStyle(native.props)}>
  <caption className="sr-only">速查表 {number}，当前页筛选结果</caption>
  <colgroup><col/>{content.columnWidths.map((width,i)=><col key={i} style={{width}}/>)}</colgroup>
  <thead>{headerRows?headers.map((items,i)=><tr key={i}>{i===0&&<th scope="col" rowSpan={headerRows}>原行号</th>}{cells(items)}</tr>):<tr><th scope="col">原行号</th>{table.headers.map((header,i)=><th scope="col" key={i}>{header}</th>)}</tr>}</thead>
  <tbody>{rows.map((items,i)=><tr key={indices[i]}><th scope="row">{indices[i]+1}</th>{cells(items)}</tr>)}</tbody>
 </table>;
}
