...
export default function ArticleEditor({initial:provided,newReference=false,newQa=false,newOps=false,fieldDefinitions=[],categoryOptions=[]}:{initial:EditorData|null;newReference?:boolean;newQa?:boolean;newOps?:boolean;fieldDefinitions?:FieldDefinition[];categoryOptions?:CategoryDefinition[]}){
 ...
 return <>{backups.map((backup,index)=><InputBackup key={index} value={backup} label={`载入前的输入备份 ${index+1}`}/>)}<ReadyEditor categoryOptions={initial?.categoryOptions??categoryOptions} key={active.generation} initial={initial} newReference={newReference} newQa={newQa} newOps={newOps} fieldDefinitions={initial?.fieldDefinitions??fieldDefinitions} nodes={nodes} hasBackups={backups.length>0} onLoad={(next,copy)=>{window.history.replaceState(null,'',`/admin/editor?article=${encodeURIComponent(next.documentId)}`);setBackups(old=>[...old,copy]);setActive(old=>({initial:next,generation:old.generation+1}));}}/></>;
}
function ReadyEditor({initial,newReference,newQa,newOps,fieldDefinitions,categoryOptions,nodes,hasBackups,onLoad}:{initial:EditorData|null;newReference:boolean;newQa:boolean;newOps:boolean;fieldDefinitions:FieldDefinition[];categoryOptions:CategoryDefinition[];nodes:EditorBlock[];hasBackups:boolean;onLoad:(data:EditorData,copy:string)=>void}){
