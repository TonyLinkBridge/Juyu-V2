...
import {categorySelection,categoryState} from '../../categories/editor';
...
import {normalizeBlocks,uploadExtensions,type ManagedAsset,type MediaBlock} from '../../media/model';
...
import {DirectPublishConfirmation} from '../review/DirectPublishConfirmation';
import {readPublication,sendPublication,publicationError,PublicationRejected} from '../../review/publication-client';
import type {PublicationInput} from '../../review/publication';
...
function ReadyEditor({recoveryOwner,initial,newReference,newQa,newOps,newTranslation,fieldDefinitions,categoryOptions,nodes,hasBackups,onLoad}:{initial:EditorData|null;recoveryOwner:string;newReference:boolean;newQa:boolean;newOps:boolean;newTranslation?:NewTranslation;fieldDefinitions:FieldDefinition[];categoryOptions:CategoryDefinition[];nodes:EditorBlock[];hasBackups:boolean;onLoad:(data:EditorData,copy:string)=>void}){
 ...
 function openSettings(section=''){setRailPanel(null);setActiveHintId(null);setSettingsSection(section);settingsDialog.current?.showModal();setSettingsOpen(true);}
 const uploadCount=useRef(0);
 const [deleting,setDeleting]=useState(false);const [reviewLocked,setReviewLocked]=useState(false);
 const [directOpen,setDirectOpen]=useState(false),[directBusy,setDirectBusy]=useState(false),[directUncertain,setDirectUncertain]=useState(false),[directPublished,setDirectPublished]=useState(false),[englishQualityConfirmed,setEnglishQualityConfirmed]=useState(false);
 const directPending=useRef<{input:PublicationInput;revision:number;previousPublication:number|null}|null>(null);
 const frozen=reviewLocked||deleting||directPublished||data?.status==='in_review'||(data!==null&&data.lifecycle!=='active');
 ...
 const touch=useEffectEvent(()=>{
  ...
  try{
   ...
   if(kind==='qa'&&presentation.tags.length>5)throw new Error('INVALID_QA');
 ...
 useEffect(()=>editor.onChange(()=>{touch();if(activeHintId&&!editor.getBlock(activeHintId)){setActiveHintId(null);setRailPanel(null);}}),[editor,activeHintId]);
 ...
 const leaving=useRef(false);const state=saver.state;const conflict=['CONFLICT','FIELD_CONFLICT','INVALID_STATE','INACTIVE_DOCUMENT'].includes(state.error);const sequence=data?.sequence??state.sequence;const dirty=state.dirty||Boolean(validation);const mustWarn=dirty||hasBackups||reviewLocked||directOpen||directUncertain;
 const directDisabled=dirty||state.busy||uploading||frozen||!data?.canDirectPublish||Boolean(validation)||directBusy||directUncertain;
 async function directPublish(){
  if(!data||sequence===null||(!directUncertain&&directDisabled))return;
  setDirectBusy(true);setNotice(null);
  try{
   let exact=directPending.current;
   if(!exact){const latest=await readPublication(documentId,sequence);if(!latest.canDirectPublish||latest.article.sequence!==sequence)throw new PublicationRejected('FORBIDDEN');const input:PublicationInput={action:'direct_publish',expectedSequence:sequence,...(locale==='en'?{englishQualityConfirmed:true as const}:{})};exact={input,revision:latest.revision,previousPublication:latest.article.publishedRevision};directPending.current=exact;}
   const ack=await sendPublication(documentId,exact.input,exact.revision,null,exact.previousPublication);directPending.current=null;setDirectUncertain(false);setDirectOpen(false);setEnglishQualityConfirmed(false);setDirectPublished(true);setData(current=>current?{...current,sequence:ack.sequence,status:'published',publishedRevision:ack.publishedRevision,canDirectPublish:false}:current);showNotice(locale==='en'?`Working revision ${ack.revision} was approved and published by your Super Admin account.`:`工作修订 ${ack.revision} 已由你的 Super Admin 账号批准并发布。`,'success');
  }catch(error){const unknown=directUncertain||!(error instanceof PublicationRejected);setDirectUncertain(unknown);if(!unknown){directPending.current=null;setDirectOpen(false);setData(current=>current?{...current,canDirectPublish:false}:current);}showNotice(publicationError(unknown?new Error('UNKNOWN_RESULT'):error,false,locale),'error');}finally{setDirectBusy(false);}
 }
 ...
 const activeHintBlock=activeHintId?editor.getBlock(activeHintId):undefined;
 let activeHint:Extract<MediaBlock,{type:'hint'}>|null=null;
 if(activeHintBlock?.type==='juyu')try{const value=normalizeBlocks([JSON.parse(activeHintBlock.props.payload)])[0];if(value.type==='hint')activeHint=value;}catch{}
 function updateActiveHint(patch:Partial<Extract<MediaBlock,{type:'hint'}>>){
  if(!activeHintBlock||!activeHint||frozen)return;
  editor.updateBlock(activeHintBlock,{props:{payload:JSON.stringify({...activeHint,...patch})}});redraw(value=>value+1);
 }
 async function deleteActiveHint(){
  if(!activeHintBlock||frozen||!await confirmAction(t('只会删除这个提示框；文章其他内容会保留。确定删除吗？','Only this callout will be removed. The rest of the article stays. Delete it?'),t('删除提示框？','Delete callout?')))return;
  editor.removeBlocks([activeHintBlock]);setActiveHintId(null);setRailPanel(null);
 }
 return <section className={`article-editor editor-focused${preview?' is-previewing':''}${railPanel?' is-tool-panel-open':''}`} aria-label={t('文章编辑器','Article editor')}>
 ...
 </div>{preview&&<section className="editor-preview" aria-label={locale==='en'?'Publication preview':'发布效果预览'}><div className="editor-preview-heading"><strong>{locale==='en'?'Publication preview':'发布效果预览'}</strong><button type="button" aria-label={locale==='en'?'Close preview':'关闭预览'} onClick={()=>setPreview(false)}><X size={18}/></button></div><h2>{title||(locale==='en'?'Untitled draft':'未命名草稿')}</h2>{kind!=='qa'&&description.trim()&&<p className="reader-page-description">{description.trim()}</p>}<p>{locale==='en'?'Draft · Not published':'当前草稿 · 尚未发布'}</p>{(frozen||!validation)&&<FieldValues fields={frozen?data?.customFields:customFields()} locale={locale}/>}{previewDocument?<>{(kind==='article'||kind==='ops')&&<nav className="editor-preview-outline" aria-label={locale==='en'?'On this page preview':'本页内容预览'}><strong>{locale==='en'?'On this page':'本页内容'}</strong>{outlineSections.length?<ol>{outlineSections.map(section=><li key={section.id} style={{paddingInlineStart:(section.depth-outlineDepth)*12}}><button type="button" onClick={event=>{const container=event.currentTarget.closest('.editor-preview');const target=Array.from(container?.querySelectorAll<HTMLElement>('.gitbook-document [id]')??[]).find(element=>element.id===section.id);if(!target)return;for(let parent=target.parentElement;parent&&parent!==container;parent=parent.parentElement){if(parent instanceof HTMLDetailsElement)parent.open=true;}target.scrollIntoView({block:'start',behavior:'auto'});target.focus({preventScroll:true});}}>{section.title||(locale==='en'?'Untitled heading':'未命名标题')}</button></li>)}</ol>:<p>{locale==='en'?'Add a heading to show an on-page outline after publication.':'正文尚未设置标题，发布后不会显示本页目录。'}</p>}</nav>}<DocumentView document={previewDocument} documentId={documentId} locale={locale} admin/></>:<p>{locale==='en'?'Fix the current content to preview it. Your draft is still here.':'当前内容有待修正，预览暂不可用。输入仍保留。'}</p>}</section>}
 </div><aside className="editor-tool-rail-shell"><nav className="editor-tool-rail" aria-label={t('编辑工具','Authoring tools')}><button type="button" aria-pressed={preview} onClick={()=>setPreview(value=>!value)}><Eye size={20}/><span>{t('预览','Preview')}</span></button><button type="button" aria-expanded={railPanel==='insert'} aria-controls="editor-insert-panel" disabled={frozen} onMouseDown={event=>event.preventDefault()} onClick={()=>{setActiveHintId(null);setRailPanel(current=>current==='insert'?null:'insert');}}><PlusCircle size={20}/><span>{t('内容插入','Insert')}</span></button><button type="button" disabled={frozen} onMouseDown={event=>event.preventDefault()} onClick={()=>{setRailPanel(null);setActiveHintId(null);openFragments();}}><Stack size={20}/><span>{t('共用片段','Snippets')}</span></button><button type="button" ref={settingsTrigger} aria-haspopup="dialog" aria-expanded={settingsOpen} onClick={()=>openSettings()}><Gear size={20}/><span>{kind==='qa'?t('问答设置','Q&A settings'):t('文章设置','Article settings')}</span></button></nav>{railPanel==='insert'&&<section id="editor-insert-panel" className="editor-tool-panel" role="region" aria-label={t('内容插入','Insert content')}><header><div><h2>{t('内容插入','Insert content')}</h2><p>{t('选择内容类型，插入到当前正文位置。','Choose a content type for the current position.')}</p></div><button type="button" aria-label={t('关闭内容插入','Close insert panel')} onClick={()=>setRailPanel(null)}><X size={20}/></button></header><div className="editor-tool-panel-content"><h3>{t('常用模块','Common blocks')}</h3><button type="button" onMouseDown={event=>event.preventDefault()} onClick={()=>add('hint')}><Info size={22}/><span><strong>{t('提示框','Callout')}</strong><small>{t('突出提醒、注意事项或操作提示','Highlight important notes or instructions')}</small></span></button><button type="button" onMouseDown={event=>event.preventDefault()} onClick={()=>{setRailPanel(null);add('code',true);}}><Code size={22}/><span><strong>{t('代码块','Code block')}</strong><small>{t('展示代码、命令或配置示例','Show code, commands or configuration')}</small></span></button><button type="button" onMouseDown={event=>event.preventDefault()} onClick={()=>{setRailPanel(null);add('table',true);}}><Table size={22}/><span><strong>{t('表格','Table')}</strong><small>{t('整理结构化资料或对比内容','Organise structured or comparison data')}</small></span></button><h3>{t('批注与说明','Notes and inline content')}</h3><button type="button" onMouseDown={event=>event.preventDefault()} onClick={()=>{setRailPanel(null);openAnnotation();}}><ChatCircleText size={22}/><span><strong>{t('添加行内注释','Add inline note')}</strong><small>{t('为选中的文字补充解释','Explain selected text')}</small></span></button><button type="button" onMouseDown={event=>event.preventDefault()} onClick={()=>{setRailPanel(null);openInline();}}><LinkSimple size={22}/><span><strong>{t('插入行内元素','Insert inline item')}</strong><small>{t('插入图标、公式或行内图片','Insert an icon, equation or inline image')}</small></span></button></div></section>}{railPanel==='hint'&&activeHint&&<section id="editor-hint-panel" className="editor-tool-panel" role="region" aria-label={t('提示框设置','Callout settings')}><header><div><h2>{t('提示框设置','Callout settings')}</h2><p>{t('正文直接在文章里编辑；这里仅调整外观。','Edit the content in the article. Use this panel only for appearance.')}</p></div><button type="button" aria-label={t('关闭提示框设置','Close callout settings')} onClick={()=>{setRailPanel(null);setActiveHintId(null);}}><X size={20}/></button></header><div className="editor-tool-panel-content editor-hint-inspector"><section><h3>{t('提示类型','Callout type')}</h3><div className="editor-hint-style-grid">{([{value:'info',zh:'信息',en:'Info'},{value:'success',zh:'成功',en:'Success'},{value:'warning',zh:'注意',en:'Warning'},{value:'danger',zh:'危险',en:'Danger'}] as const).map(option=><button key={option.value} type="button" aria-pressed={activeHint.style===option.value} onClick={()=>updateActiveHint({style:option.value})}><span aria-hidden="true" data-style={option.value}/>{locale==='en'?option.en:option.zh}</button>)}</div></section><section><h3>{t('图标','Icon')}</h3><label><span className="sr-only">{t('提示图标','Callout icon')}</span><select aria-label={t('提示图标','Callout icon')} value={activeHint.iconKey??''} disabled={frozen} onChange={event=>updateActiveHint({iconKey:event.target.value?event.target.value as ReaderIconKey:null})}><option value="">{t('跟随提示类型','Match callout type')}</option>{Object.entries(readerIconLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label></section><section><h3>{t('标题','Title')}</h3><label className="editor-hint-title-toggle"><input type="checkbox" aria-label={t('显示标题','Show title')} checked={activeHint.showTitle!==false} disabled={frozen} onChange={event=>updateActiveHint({showTitle:event.target.checked})}/><span>{t('显示标题','Show title')}</span></label><p>{t('关闭后只隐藏标题，原有标题文字仍会保留。','Turning this off hides the title without deleting it.')}</p></section><button className="editor-hint-delete" type="button" disabled={frozen} onClick={()=>void deleteActiveHint()}>{t('删除这个提示框','Delete this callout')}</button></div></section>}</aside><dialog ref={settingsDialog} className="editor-settings-drawer" aria-labelledby="editor-settings-title" onClose={()=>{setSettingsOpen(false);settingsTrigger.current?.focus();}}><div className="editor-settings-heading"><h2 id="editor-settings-title">{panelNames[settingsSection]??t('文章设置','Article settings')}</h2><button type="button" aria-label={t('关闭文章设置','Close article settings')} onClick={()=>settingsDialog.current?.close()}><X size={20}/></button></div><div className="editor-settings-content">{settingsSection?<button className="editor-settings-back" type="button" onClick={()=>setSettingsSection('')}><ArrowLeft size={16}/>{t('全部设置','All settings')}</button>:<nav className="editor-settings-menu" aria-label={t('文章设置项目','Article setting options')}>{Object.entries(panelNames).map(([key,label])=><button key={key} type="button" onClick={()=>setSettingsSection(key)}><strong>{label}</strong><span>{locale==='en'?{basic:kind==='qa'?'Set the content type, access and related topics':'Set the content type, access and tags',release:'Add a note for the next published version',categories:'Choose where this article appears',files:'Upload files and choose a cover',fields:'Fill in team-specific details',actions:'Save a backup, view history or move to trash'}[key]:{basic:kind==='qa'?'设置资料类型、阅读权限和相关话题':'设置资料类型、阅读权限和标签',release:'撰写二审通过后显示的版本说明',categories:'选择文章在资料库中的位置',files:'上传文件，选择文章封面',fields:'填写团队设置的补充资料',actions:'备份输入、查看历史或移入回收站'}[key]}</span></button>)}</nav>}<section hidden={settingsSection!=='basic'} data-section="basic"> <fieldset disabled={frozen}><legend>{t('文章资料','Article details')}</legend><div className="editor-metadata"><label>{t('资料类型','Content type')}<select value={kind} disabled={data!==null||state.busy||newQa||newOps||Boolean(newTranslation)} onChange={e=>{const v=e.target.value as ContentKind;setKind(v);if(v==='ops'&&audience==='staff')setAudience('ops');}}>{Object.entries(kinds).map(([v,n])=><option key={v} value={v}>{locale==='en'?{article:'Knowledge article',ops:'OPS Internal',reference:'Reference',qa:'Q&A'}[v]??n:n}</option>)}</select></label><label>{t('阅读范围','Who can read this')}<select value={audience} onChange={e=>setAudience(e.target.value as Audience)}>{kind!=='ops'&&<option value="staff">{t('全体员工（客服、运营、管理员）','All staff (Support, Ops and Admin)')}</option>}<option value="ops">{t('运营和管理员','Ops and Admin')}</option><option value="admin">{t('仅管理员','Admin only')}</option></select></label><div><label>{kind==='qa'?t('相关话题（最多 5 个）','Related topics (up to 5)'):t('文章标签','Tags')}<input maxLength={500} value={tags} onChange={e=>setTags(e.target.value)} placeholder={kind==='qa'?t('例如：签约店铺，信用额度','For example: Store contract, credit limit'):t('例如：域名转入，客户验证','For example: Domain transfer, customer verification')}/></label>{kind==='qa'&&<small>{t('用逗号分隔。员工可以按任一话题找到这条问答；主要分类仍决定它的主要归属。','Separate topics with commas. Employees can find this Q&A through any topic; the main category remains its primary home.')}</small>}</div></div></fieldset>
...
</section><section hidden={settingsSection!=='fields'} data-section="fields"> {frozen?<FieldValues fields={data?.customFields} locale={locale}/>:<FieldInputs definitions={definitions} previous={initial?.customFields??[]} values={fieldInputs} onChange={(id,value)=>setFieldInputs(old=>({...old,[id]:value}))} disabled={frozen} locale={locale}/>}
