'use client';
import {Gear,Eye,X,ImageSquare,DotsThree,ArrowLeft} from '@phosphor-icons/react';
...
function ReadyEditor({initial,newReference,newQa,fieldDefinitions,categoryOptions,nodes,hasBackups,onLoad}:{initial:EditorData|null;newReference:boolean;newQa:boolean;fieldDefinitions:FieldDefinition[];categoryOptions:CategoryDefinition[];nodes:EditorBlock[];hasBackups:boolean;onLoad:(data:EditorData,copy:string)=>void}){
 ...
 function openSettings(section?:string){const dialog=settingsDialog.current;if(!dialog)return;dialog.showModal();setSettingsOpen(true);if(section){const target=dialog.querySelector<HTMLDetailsElement>(`[data-section="${section}"]`);if(target){target.open=true;target.scrollIntoView({block:'start'});}}}
 ...
 useEffect(()=>{if(!mustWarn&&!state.busy)return;const warn=(e:BeforeUnloadEvent)=>{if(leaving.current)return;e.preventDefault();e.returnValue='';};const link=(e:MouseEvent)=>{const a=(e.target as Element).closest?.('a[href]');if(a?.closest('.bn-editor[contenteditable="true"]'))return;if(a&&!window.confirm('页面有未保存输入或恢复备份，确定离开吗？')){e.preventDefault();e.stopPropagation();}};window.addEventListener('beforeunload',warn);document.addEventListener('click',link,true);return()=>{window.removeEventListener('beforeunload',warn);document.removeEventListener('click',link,true);};},[mustWarn,state.busy]);
 ...
 <div className="editor-toolbar"><a className="editor-back" href="/admin"><ArrowLeft size={17}/>内容管理</a><span className="editor-page-label">{providedLabel(data)}</span><p role="status" className={state.error?'save-state has-error':'save-state'}>{state.busy?'正在保存…':state.error?saveError(state.error):dirty?'有未保存修改':data?'所有修改已保存':'草稿 · 自动保存'}</p>
 <div className="editor-toolbar-actions">{state.error&&<button type="button" disabled={frozen||state.busy||!state.dirty||Boolean(validation)} onClick={()=>void saver.save(true)}>{state.error?'重试保存':'立即保存'}</button>}<button type="button" className="editor-preview-toggle" aria-pressed={preview} onClick={()=>setPreview(v=>!v)}><Eye size={17}/>{preview?'关闭草稿预览':'预览草稿'}</button><button type="button" ref={settingsTrigger} aria-haspopup="dialog" aria-expanded={settingsOpen} onClick={()=>openSettings()}><Gear size={17}/>文章设置</button>{(!data||data.lifecycle==='active')&&<SubmitReview compact id={documentId} sequence={sequence} disabled={dirty||state.busy||uploading||frozen||!data||Boolean(data&&data.status!=='draft')} onLock={setReviewLocked} onSubmitted={result=>{setData(current=>current?{...current,sequence:result.sequence,status:'in_review'}:current);}}/>}</div></div>
 ...
 {validation&&<p role="alert">{validation}</p>}{frozen&&<p role="alert">{reviewLocked?'正在核对二审提交结果，编辑暂时暂停。请在二审窗口完成核对。':data?.status==='in_review'?'已提交二审，本次版本暂时锁定。已有正式版本继续可读。':'当前内容已停用，不能编辑。请先完成相应流程。'}</p>}<p>{notice}</p>{data&&(data.publishedRevision!==null||data.status!=='draft')&&<p className="editor-published-note">{publicationLabel({revision:0,publishedRevision:data.publishedRevision,status:data.status as 'draft'})}</p>}
 {data&&data.publishedRevision!==null&&<p className="editor-publication-guidance">修改会保存为新草稿，旧正式版继续可读；新稿需要重新二审和发布。</p>}

 </div><div className="editor-workarea"><div className="editor-writing"><div className="editor-document-heading"><button className="editor-cover-trigger" type="button" onClick={()=>openSettings('files')}><ImageSquare size={17}/>{cover?'管理封面':'添加封面'}</button><span className="editor-draft-label">{{draft:'草稿',in_review:'等待二审',changes_requested:'需要修改',approved:'已经批准',queued:'等待发布',published:'已经发布'}[data?.status??'draft']??data?.status}</span><label className="editor-title-label"><span className="sr-only">文章标题</span><textarea ref={titleElement} aria-label="文章标题" placeholder="请输入文章标题" rows={1} maxLength={200} disabled={frozen} value={title} onChange={e=>setTitle(e.target.value)}/></label><div className="editor-metadata-summary"><button type="button" onClick={()=>openSettings('categories')}>分类：{categoryIds.length?categoryIds.map(id=>categoryOptions.find(c=>c.id===id)?.name??'原有分类').slice(0,2).join('、')+(categoryIds.length>2?` 等 ${categoryIds.length} 项`:''):'未分类'}</button><button type="button" onClick={()=>openSettings('basic')}>阅读：{{staff:'普通员工',ops:'Ops 与 Admin',admin:'仅 Admin'}[audience]}</button><button type="button" aria-label="更多文章设置" onClick={()=>openSettings()}><DotsThree size={20}/></button></div></div>
 <EditorContext.Provider value={{documentId,assets,frozen}}><div className="editor-canvas"><BlockNoteView editor={editor} editable={!frozen} theme={theme} slashMenu={false}>
...
</details><details open data-section="categories"><summary>目录分类</summary> {frozen?<ArticleCategories options={data?.categoryOptions} ids={data?.categoryIds}/>:<CategoryInputs options={categoryOptions} ids={categoryIds} disabled={frozen} onChange={setCategoryIds}/>}
</details><details data-section="files"><summary>封面与附件</summary> <fieldset disabled={frozen||uploading}><legend>私有文件与封面</legend><label>上传文件<input type="file" disabled={!data} accept=".png,.jpg,.jpeg,.gif,.webp,.mp4,.webm,.mp3,.ogg,.pdf,.txt,.csv" onChange={e=>{const f=e.target.files?.[0];e.target.value='';if(f)void upload(f).catch(()=>{});}}/></label>{!data&&<p>先保存草稿，即可上传本篇文件。</p>}<label>选择本篇文件<select value={selected} onChange={e=>setSelected(e.target.value)}><option value="">请选择文件</option>{assets.filter(a=>a.status==='ready').map(a=><option key={a.id} value={a.id}>{a.filename}</option>)}</select></label><div className="media-toolbar"><button type="button" disabled={!selected} onClick={()=>{const a=assets.find(a=>a.id===selected);if(a)add(a.mime.startsWith('image/')?'image':a.mime.startsWith('video/')?'video':a.mime.startsWith('audio/')?'audio':'file');}}>插入文件到正文</button></div><label>文章封面<select value={cover?.assetId??''} onChange={e=>setCover(e.target.value?{assetId:e.target.value,alt:'',position:50}:null)}><option value="">不使用封面</option>{assets.filter(a=>a.status==='ready'&&a.mime.startsWith('image/')).map(a=><option key={a.id} value={a.id}>{a.filename}</option>)}</select></label>{cover&&<><label>封面替代文字<input value={cover.alt} maxLength={200} onChange={e=>setCover({...cover,alt:e.target.value})}/></label><label>封面位置<input type="range" min={0} max={100} value={cover.position} onChange={e=>setCover({...cover,position:Number(e.target.value)})}/></label></>}</fieldset>
</details><details data-section="fields"><summary>更多字段</summary> {frozen?<FieldValues fields={data?.customFields}/>:<FieldInputs definitions={definitions} previous={initial?.customFields??[]} values={fieldInputs} onChange={(id,value)=>setFieldInputs(old=>({...old,[id]:value}))} disabled={frozen}/>}
</details><details data-section="actions"><summary>更多操作</summary><button type="button" disabled={frozen||state.busy||!state.dirty||Boolean(validation)} onClick={()=>void saver.save(true)}>{state.error?'重试保存':'立即保存'}</button><button type="button" onClick={async()=>{try{await navigator.clipboard.writeText(copy);setNotice('当前输入已复制。');}catch{setNotice('无法复制，请保留此页面并手动复制输入。');}}}>复制当前输入</button><button type="button" disabled={state.busy} onClick={()=>{if(!mustWarn||confirm('刷新会清除页面内未保存输入及恢复备份，确定继续？')){leaving.current=true;window.location.reload();}}}>重新载入</button> {data&&!reviewLocked&&<p className="editor-review-link"><a href={`/admin/review?article=${encodeURIComponent(documentId)}`}>查看二审详情与退回原因</a><a href={`/admin/availability?article=${encodeURIComponent(documentId)}`}>归档与下线</a><a href={`/admin/history?article=${encodeURIComponent(documentId)}`}>历史记录与版本</a>{data.status==='changes_requested'&&' · 请先按退回原因修改并保存，再重新提交二审。'}</p>} {data&&<DocumentTrashAction id={documentId} title={title} sequence={sequence??data.sequence} disabled={dirty||state.busy||uploading||reviewLocked||data.lifecycle!=='active'} onBusy={setDeleting} onDeleted={sequence=>{setData({...data,sequence,lifecycle:'trashed',status:'draft',publishedRevision:null});}}/>}</details></div></dialog>
 </section>;
}
