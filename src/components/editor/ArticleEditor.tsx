...
function ReadyEditor({recoveryOwner,initial,newReference,newQa,newOps,fieldDefinitions,categoryOptions,nodes,hasBackups,onLoad}:{initial:EditorData|null;recoveryOwner:string;newReference:boolean;newQa:boolean;newOps:boolean;fieldDefinitions:FieldDefinition[];categoryOptions:CategoryDefinition[];nodes:EditorBlock[];hasBackups:boolean;onLoad:(data:EditorData,copy:string)=>void}){
 ...
 </div>{preview&&<section className="editor-preview" aria-label="发布效果预览"><div className="editor-preview-heading"><strong>发布效果预览</strong><button type="button" aria-label="关闭预览" onClick={()=>setPreview(false)}><X size={18}/></button></div><h2>{title||'未命名草稿'}</h2><p>当前草稿 · 尚未发布</p>{(frozen||!validation)&&<FieldValues fields={frozen?data?.customFields:customFields()}/>}{previewDocument?<>{(kind==='article'||kind==='ops')&&<nav className="editor-preview-outline" aria-label="本页内容预览"><strong>本页内容</strong>{outlineSections.length?<ol>{outlineSections.map(section=><li key={section.id} style={{paddingInlineStart:(section.depth-outlineDepth)*12}}><button type="button" onClick={event=>{const container=event.currentTarget.closest('.editor-preview');const target=Array.from(container?.querySelectorAll<HTMLElement>('.gitbook-document [id]')??[]).find(element=>element.id===section.id);if(!target)return;for(let parent=target.parentElement;parent&&parent!==container;parent=parent.parentElement){if(parent instanceof HTMLDetailsElement)parent.open=true;}target.scrollIntoView({block:'start',behavior:'auto'});target.focus({preventScroll:true});}}>{section.title||'未命名标题'}</button></li>)}</ol>:<p>正文尚未设置标题，发布后不会显示本页目录。</p>}</nav>}<DocumentView document={previewDocument} documentId={documentId} admin/></>:<p>当前内容有待修正，预览暂不可用。输入仍保留。</p>}</section>}
