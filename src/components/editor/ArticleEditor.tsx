...
function ReadyEditor({initial,newReference,newQa,fieldDefinitions,categoryOptions,nodes,hasBackups,onLoad}:{initial:EditorData|null;newReference:boolean;newQa:boolean;fieldDefinitions:FieldDefinition[];categoryOptions:CategoryDefinition[];nodes:EditorBlock[];hasBackups:boolean;onLoad:(data:EditorData,copy:string)=>void}){
 ...
 </div><div className="editor-workarea"><div className="editor-writing"><div className="editor-document-heading"><button className="editor-cover-trigger" type="button" onClick={()=>openSettings('files')}><ImageSquare size={17}/>{cover?'管理封面':'添加封面'}</button><span className="editor-draft-label">{{draft:'草稿',in_review:'等待二审',changes_requested:'需要修改',approved:'已经批准',queued:'等待发布',published:'已经发布'}[data?.status??'draft']??data?.status}</span><label className="editor-title-label"><span className="sr-only">文章标题</span><textarea ref={titleElement} aria-label="文章标题" placeholder="请输入文章标题" rows={1} maxLength={200} disabled={frozen} value={title} onChange={e=>setTitle(e.target.value)}/></label>{!title.trim()&&<p className="editor-title-hint">先给文章起个标题，之后修改会自动保存。</p>}<div className="editor-metadata-summary"><button type="button" onClick={()=>openSettings('categories')}>{categoryIds.length?'分类：'+categoryIds.map(id=>categoryOptions.find(c=>c.id===id)?.name??'原有分类').slice(0,2).join('、')+(categoryIds.length>2?` 等 ${categoryIds.length} 项`:''):'添加分类'}</button><button type="button" onClick={()=>openSettings('basic')}>谁可以阅读：{{staff:'全体员工',ops:'运营和管理员',admin:'仅管理员'}[audience]}</button><button type="button" aria-label="更多文章设置" onClick={()=>openSettings()}>更多设置</button></div></div>
