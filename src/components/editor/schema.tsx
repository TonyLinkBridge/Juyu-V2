...
const juyu=createReactBlockSpec({type:'juyu',propSchema:{payload:{default:''}},content:'none'}, {
 render:function EmbeddedBlock({block,editor}){
  ...
  try{payload=JSON.parse(block.props.payload) as Record<string,unknown>;}catch{return <div role="alert">内容块无法读取。<button type="button" disabled={context.frozen} onClick={()=>editor.removeBlocks([block])}>删除内容块</button></div>;}
  if(payload.type==='reusableContent')return <div className="editor-embedded" data-juyu-type="reusableContent" contentEditable={false}><strong>共用片段 · {String(payload.title)} · 第 {String(payload.version)} 版</strong><p className="editor-embedded-note">下方是已插入本文的内容副本。更新片段请使用顶部「共用片段」，保存后仍需审核。</p><div className="media-toolbar"><button type="button" disabled={context.frozen} onClick={()=>editor.moveBlocksUp(block)}>上移片段</button><button type="button" disabled={context.frozen} onClick={()=>editor.moveBlocksDown(block)}>下移片段</button><button type="button" disabled={context.frozen} onClick={()=>editor.removeBlocks([block])}>删除片段</button></div></div>;
  const media={...payload,id:block.id} as MediaBlock;
  try{normalizeBlocks([media]);}catch{invalid=true;}
  const names={image:'图片',video:'影片',audio:'音频',file:'文件',table:'表格',hint:'提示框',code:'代码框',tabs:'分页标签',steps:'操作步骤',columns:'分栏布局',articleReference:'引用文章',button:'操作按钮',externalEmbed:'外部内容',math:'数学公式',diagram:'流程图'};
  return <div className="editor-embedded" data-juyu-type={media.type} data-juyu-style={media.type==='hint'?media.style:undefined} contentEditable={false}><strong>{names[media.type]}</strong>{invalid&&<p role="alert">此内容块尚未符合保存要求，请修正下方输入。</p>}<details><summary>编辑此内容块</summary><fieldset disabled={context.frozen}><MediaFields block={media} assets={context.assets} documentId={context.documentId} frozen={context.frozen} locale={context.locale} onChange={next=>editor.updateBlock(block,{props:{payload:JSON.stringify(next)}})}/></fieldset></details>
   {!invalid&&(media.type==='hint'?<p className="editor-embedded-note">完整提示框请使用顶部的「预览草稿」查看。</p>:<details><summary>查看此块预览</summary><MediaBlocks blocks={[media]} documentId={context.documentId} locale={context.locale} admin/></details>)}
   <div className="media-toolbar">{media.type==='hint'&&<button type="button" disabled={context.frozen} onClick={()=>editor.updateBlock(block,{children:[...block.children,{type:'paragraph',content:''}] as never})}>在提示框中添加段落</button>}<button type="button" disabled={context.frozen} onClick={()=>editor.moveBlocksUp(block)}>上移内容块</button><button type="button" disabled={context.frozen} onClick={()=>editor.moveBlocksDown(block)}>下移内容块</button><button type="button" disabled={context.frozen} onClick={()=>editor.removeBlocks([block])}>删除内容块</button></div>
  </div>;
 },
