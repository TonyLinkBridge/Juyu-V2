...
import {displayColor,color} from '../../editor/inline';
...
function nativeColor<const T extends 'textColor'|'backgroundColor'>(type:T){
 ...
 const render=(value:string)=>{const span=document.createElement('span');if(!nativeNames.has(value)){try{span.style[property]=color(value);}catch{ /* Invalid clipboard styles remain inert until document validation. */ }}return {dom:span,contentDOM:span};};
