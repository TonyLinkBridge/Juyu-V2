...
const links=[['/admin','知识文章',Files],['/admin?kind=ops&view=list','OPS 内容管理',Files],['/admin?kind=qa&view=list','Q&A 管理',ChatCircle],['/admin?scope=review&view=list','待我审核',Clock],['/admin/media','媒体文件',ImageSquare],['/admin/members','成员与权限',Users],['/admin/analytics','使用分析',ChartBar],['/admin/feedback','文章反馈',ChatCircle],['/admin/availability','归档资料',Archive],['/admin/trash','回收站',Trash]] as const;
...
function Sidebar({path}:{path:string}){const params=useSearchParams();const reviewing=path==='/admin'&&params.get('scope')==='review';return <nav aria-label="后台导航"><div className="sidebar-nav-group">{links.map(([href,label,Icon])=><NavigationLink prefetch={false} href={href} key={href} aria-current={(href.includes('kind=')?path==='/admin'&&!reviewing&&params.get('kind')===new URLSearchParams(href.split('?')[1]).get('kind'):href.includes('?')?reviewing:href===path&&!(href==='/admin'&&(reviewing||['qa','ops'].includes(params.get('kind')??''))))?'page':undefined}><Icon size={20}/>{label}</NavigationLink>)}</div><details className="settings-nav" open={path.startsWith('/admin/settings')||path==='/admin/forms'||path==='/admin/announcements'}><summary><Gear size={20}/>设置</summary>{[['features','功能开关'],['navigation','导航设置'],['categories','分类设置'],['fields','自定义字段'],['forms','自定义表单'],['history','设置变更记录']].map(([id,title])=><NavigationLink prefetch={false} key={id} href={`/admin/settings/${id}`} aria-current={path===`/admin/settings/${id}`?'page':undefined}>{title}</NavigationLink>)}<NavigationLink prefetch={false} href="/admin/announcements">公告管理</NavigationLink><NavigationLink prefetch={false} href="/admin/forms">表单提交记录</NavigationLink></details></nav>;}
