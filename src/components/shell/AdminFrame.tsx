...
function Sidebar({path}:{path:string}){const params=useSearchParams();const reviewing=path==='/admin'&&params.get('scope')==='review';return <nav aria-label="后台导航"><div className="sidebar-nav-group">{links.map(([href,label,Icon])=><Link prefetch={false} href={href} key={href} aria-current={(href.includes('?')?reviewing:href===path&&!(href==='/admin'&&reviewing))?'page':undefined}><Icon size={20}/>{label}</Link>)}</div><details className="settings-nav" open={path.startsWith('/admin/settings')||path==='/admin/forms'||path==='/admin/announcements'}><summary><Gear size={20}/>设置</summary>{[['features','功能开关'],['navigation','导航设置'],['categories','分类设置'],['fields','自定义字段'],['forms','自定义表单'],['history','设置变更记录']].map(([id,title])=><Link prefetch={false} key={id} href={`/admin/settings/${id}`} aria-current={path===`/admin/settings/${id}`?'page':undefined}>{title}</Link>)}<Link prefetch={false} href="/admin/announcements">公告管理</Link><Link prefetch={false} href="/admin/forms">表单提交记录</Link></details></nav>;}
