...
function Popover({label,children,className='',locale='zh-CN',showMobileAppearance=true}:{label:ReactNode;children:ReactNode;className?:string;locale?:'zh-CN'|'en';showMobileAppearance?:boolean}){
 ...
 return <details ref={ref} className={`account-menu ${className}`}><summary>{label}</summary><div>{showMobileAppearance&&<div className="mobile-account-appearance"><p>{locale==='en'?'Appearance':'外观主题'}</p><ThemeSwitch mode="light-dark-system"/></div>}{children}</div></details>;
}
