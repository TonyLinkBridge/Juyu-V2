...
function Popover({label,children,className=''}:{label:ReactNode;children:ReactNode;className?:string}){
 ...
 return <details ref={ref} className={`account-menu ${className}`}><summary>{label}</summary><div><div className="mobile-account-appearance"><p>外观主题</p><ThemeToggler compact/></div>{children}</div></details>;
}
...
export function AccountMenu({admin=false,enabled=false}:{admin?:boolean;enabled?:boolean}){
 ...
 return <div className="account-controls"><ThemeToggler compact/>{preview?<>{!admin&&<AdminShortcut/>}<Popover label={<AccountIdentity name="tony" role="Admin"/>}>{!admin&&<div className="mobile-account-shortcut"><AdminShortcut/></div>}<strong>示例账号</strong><p>仅用于界面预览，未代表任何真实登录身份。</p><p>正式登录后可查看姓名、邮箱和角色，并管理账号或退出。</p></Popover></>:enabled?<SignedInAccount admin={admin}/>:<Popover label={<AccountIdentity name="账号" role="未登录"/>}><p>登录服务尚未配置，账号操作暂不可用。</p></Popover>}</div>;
}
