...
function SignedInAccount({admin,locale}:{admin:boolean;locale:'zh-CN'|'en'}){
 ...
 const localizedRole=locale==='en'?(role==='admin'?'Admin':role==='ops'?'Ops':role==='support'?'Support':'Access pending'):roleLabel;
 return <>{!admin&&isLoaded&&isSignedIn&&role==='admin'&&<AdminShortcut locale={locale}/>}<Popover locale={locale} label={<AccountIdentity name={isLoaded&&isSignedIn?name:(locale==='en'?'Account':'账号')} role={isLoaded&&isSignedIn?localizedRole:!isLoaded?(locale==='en'?'Checking…':'正在确认…'):(locale==='en'?'Signed out':'未登录')} locale={locale}/>}>
