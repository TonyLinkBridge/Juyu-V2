...
function Popover({label,children,className='',locale='zh-CN',showMobileAppearance=true}:{label:ReactNode;children:ReactNode;className?:string;locale?:'zh-CN'|'en';showMobileAppearance?:boolean}){
 ...
 return <details ref={ref} className={`account-menu ${className}`}><summary>{label}</summary><div>{showMobileAppearance&&<div className="mobile-account-appearance"><p>{locale==='en'?'Appearance':'外观主题'}</p><ThemeToggler compact locale={locale}/></div>}{children}</div></details>;
}
...
export function AccountMenu({admin=false,enabled=false,locale='zh-CN',accountOnly=false}:{admin?:boolean;enabled?:boolean;locale?:'zh-CN'|'en';accountOnly?:boolean}){
 ...
 return <div className="account-controls">{!accountOnly&&!admin&&<LocaleSwitch locale={locale}/>} {!accountOnly&&<ThemeToggler compact locale={locale}/>} {preview?<>{!admin&&<AdminShortcut locale={locale}/>}<Popover locale={locale} showMobileAppearance={!accountOnly} label={<AccountIdentity name="tony" role="Admin" locale={locale}/>}>{!admin&&<div className="mobile-account-shortcut"><AdminShortcut locale={locale}/></div>}<strong>{locale==='en'?'Demo account':'示例账号'}</strong><p>{locale==='en'?'For interface previews only. This is not a real signed-in identity.':'仅用于界面预览，未代表任何真实登录身份。'}</p><p>{locale==='en'?'After signing in, you can view your name, email and role, manage your account, or sign out.':'正式登录后可查看姓名、邮箱和角色，并管理账号或退出。'}</p></Popover></>:enabled?<SignedInAccount admin={admin} locale={locale} accountOnly={accountOnly}/>:<Popover locale={locale} showMobileAppearance={!accountOnly} label={<AccountIdentity name={locale==='en'?'Account':'账号'} role={locale==='en'?'Signed out':'未登录'} locale={locale}/>}><p>{locale==='en'?'Account controls are unavailable because sign-in is not configured.':'登录服务尚未配置，账号操作暂不可用。'}</p></Popover>}</div>;
}
function SignedInAccount({admin,locale,accountOnly=false}:{admin:boolean;locale:'zh-CN'|'en';accountOnly?:boolean}){
 ...
 return <>{!admin&&isLoaded&&isSignedIn&&administrator&&<AdminShortcut locale={locale}/>}<Popover locale={locale} showMobileAppearance={!accountOnly} label={<AccountIdentity name={isLoaded&&isSignedIn?name:(locale==='en'?'Account':'账号')} role={isLoaded&&isSignedIn?localizedRole:!isLoaded?(locale==='en'?'Checking…':'正在确认…'):(locale==='en'?'Signed out':'未登录')} locale={locale}/>}>
