...
export function pdfContent(article:Publication,coverSource?:string,images?:Record<string,string>):string {
 ...
 return `${coverSource?`<img class="pdf-cover" src="${e(coverSource)}" alt="${e(article.cover?.alt??'')}" style="object-position:center ${article.cover?.position??50}%">`:''}<h1>${e(article.title)}</h1><p class="pdf-meta">JUYU 内部资料库 · ${article.publicationNumber?`正式版本 ${article.publicationNumber}`:'已发布'}${article.tags?.length?` · ${article.tags.map(e).join(' / ')}`:''}</p>${fields}${blocks||(!media?'<p>这篇文章暂时没有正文。</p>':'')}${media}`;
}
