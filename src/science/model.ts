...
export function mathMarkup(source:string,displayMode=true):string {
 ...
 try{return katex.renderToString(source,{displayMode,output:'mathml',throwOnError:true,trust:false,strict:'error',maxExpand:100,maxSize:20});}catch(error){throw new Error((error instanceof Error?error.message:'公式格式错误').slice(0,350));}
}
