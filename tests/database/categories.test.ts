...
test('membership rejects unknown disabled ancestors duplicate overflow raw association and unbound drafts',async()=>{
 ...
 for(const categoryIds of [[randomUUID()],[child.id,child.id],Array.from({length:21},()=>randomUUID())])await assert.rejects(repo.saveEditor(id,{...input,expectedSequence:saved.sequence,categoryIds},admin),/INVALID_INPUT/);
