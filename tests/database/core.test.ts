...
test('empty database migration is replayable without losing data', async () => {
  ...
  assert.equal((await fixture.pool.query('SELECT count(*)::int AS n FROM juyu.schema_migrations')).rows[0].n, 45);
});
  ...
  test(`two simultaneous ${operation} operations have exactly one winner`, async () => {
    ...
    try {
      ...
      while (Date.now() < deadline) {
        waiting = (await fixture.pool.query("SELECT count(*)::int AS n FROM pg_stat_activity WHERE wait_event_type='Lock' AND query LIKE 'SELECT id,locale FROM juyu.documents%' ")).rows[0].n;
        if (waiting >= 2) break;
...
test('migration rollback and reapply is exercised only on a separate disposable cluster', async () => {
  ...
  try {
    assert.equal((await migrate(isolated.pool)).length, 45);
