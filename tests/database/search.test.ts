...
test('search ranks exact titles before title phrases tags and body matches',async()=>{
 await publish(await draft('rankingtarget only in body','article','A body match'));
 await publish(await draft('ordinary body','article','B tag match',['rankingtarget']));
 await publish(await draft('ordinary body','article','Z rankingtarget guide'));
 await publish(await draft('ordinary body','article','rankingtarget'));
 const result=(await service().search('rankingtarget')).search;
 assert.deepEqual(result.results.map(item=>item.title),[
  'rankingtarget','Z rankingtarget guide','B tag match','A body match',
 ]);
});
