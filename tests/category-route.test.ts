...
const tree=[{type:'group' as const,id,title:'账户管理',descendants:[{type:'document' as const,id:'article-one',title:'账户安全',href:'/help-centre?article=article-one'}]}];
async function route({nodes=tree,error='',entryError=false}={}){
 ...
 const stub=(name:string)=>{
  ...
  if(name.endsWith('/authorization/application'))return {applicationAuthorization:async()=>({categoryNavigationTree:async()=>{if(error)throw new Error(error);return nodes;}})};
...
test('category route directly opens authorized reading without catching Next redirect',async()=>{
 const page=await route();await assert.rejects(()=>page(params()),/REDIRECT:\/help-centre\?article=article-one/);
});
test('missing or inaccessible category keeps unavailable state without disclosing title',async()=>{
 const page=await route({nodes:[]});const result=await page(params());const output=JSON.stringify(result);assert.ok(!output.includes('账户管理'));assert.ok(output.includes('"failed":false'));assert.ok(output.includes('"account":true'));
});
test('category service failure is distinguished from empty and entry denial is not swallowed',async()=>{
 const page=await route({error:'SERVICE_UNAVAILABLE'});assert.ok(JSON.stringify(await page(params())).includes('"failed":true'));
...
test('invalid category and repeated page input never redirect into broader collection',async()=>{
 const page=await route();for(const args of [params('../private'),params(id,{page:['1','2']})])assert.ok(JSON.stringify(await page(args)).includes('"data":null'));
});
