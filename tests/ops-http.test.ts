...
test('OPS request pages are exact and private failures do not leak service details',async()=>{
 ...
 assert.equal(m.opsLocale(new URL('http://local/?lang=en')),'en');assert.equal(m.opsLocale(new URL('http://local/')),'zh-CN');
 for(const query of ['lang=fr','lang=en&lang=en'])assert.throws(()=>m.opsLocale(new URL(`http://local/?${query}`)),/INVALID_INPUT/);
