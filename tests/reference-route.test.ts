...
async function route(error='',empty=false){
 ...
 if(name.endsWith('/authorization/application'))return {applicationAuthorization:async()=>({reference:async()=>({items:empty?[]:[{id:'00000000-0000-4000-8000-000000000001'}],total:empty?0:1,page:1,pages:1}),referenceDetail:async(id:string)=>{if(error)throw new Error(error);return {id,title:'费用',revision:1,tables:[]};}})};
