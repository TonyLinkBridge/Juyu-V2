...
test('Reference query rejects ambiguous page or article and responses never share private errors',async()=>{
 assert.deepEqual(referenceQuery(new URL('http://local/?page=2&article=ref')), {page:2,article:'ref'});
 for(const q of ['?page=0','?page=1&page=2','?article=a&article=b','?role=admin','?article=','?article=%00','?page=1e3'])assert.throws(()=>referenceQuery(new URL('http://local/'+q)),/INVALID_INPUT/);
