...
test('unified results expose only server-provided escaped snippets and document metadata',async()=>{
 ...
 assert.ok(html.includes('search-result-snippet'));assert.ok(html.includes('&lt;script&gt;'));assert.ok(!html.includes('<script>'));assert.ok(html.includes('Reference'));assert.ok(html.includes('费用标签'));assert.ok(html.includes('已发布'));assert.ok(html.includes('href="/help-centre?article=one"'));assert.ok(html.includes('标题、标签和正文'));
