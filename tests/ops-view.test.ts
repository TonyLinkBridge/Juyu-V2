...
test('OPS collection distinguishes authorized content empty denied and unavailable without leaked metadata',async()=>{
 ...
 const html=renderToStaticMarkup(createElement(m.OpsCollection,{data,state:'ready'}));assert.match(html,/受限标题 &lt;script&gt;/);assert.match(html,/\/help-centre\/articles\/ops%2Fid/);assert.match(html,/\/help-centre\/ops\?page=2/);assert.match(html,/内部标签/);
