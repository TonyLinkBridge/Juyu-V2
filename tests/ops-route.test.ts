...
async function route(error='',empty=false){
 ...
 if(name.endsWith('/reader/content-path'))return {articleContentPath:(id:string)=>'/help-centre/articles/'+encodeURIComponent(id)};
...
test('OPS entry opens authorized article directly and preserves redirect',async()=>{const page=await route();await assert.rejects(()=>page({searchParams:Promise.resolve({})}),/REDIRECT:\/help-centre\/articles\/ops%20%26%20one/);});
