...
test('Q&A query distinguishes all and unclassified and rejects ambiguous inputs',()=>{
 ...
 assert.deepEqual(qaQuery(new URL('http://local/?topic='+encodeURIComponent('签约店铺'))),{page:1,topic:'签约店铺',locale:'zh-CN'});
 for(const q of ['?page=0','?page=1&page=2','?category=a&category=b','?topic=a&topic=b','?role=admin','?category=%00','?topic=%00','?page=1e3','?lang=fr','?lang=en&lang=en','?category='+encodeURIComponent('字'.repeat(81)),'?topic='+encodeURIComponent('字'.repeat(41))])assert.throws(()=>qaQuery(new URL('http://local/'+q)),/INVALID_INPUT/);
});
