...
test('PDF reading keeps Chinese tables, attachment controls and print layout on desktop and mobile',async({page},info)=>{
 ...
 for(const mode of ['light','dark'] as const){await page.emulateMedia({colorScheme:mode});await expect(page.locator('html')).toHaveAttribute('data-theme',mode);const action=page.getByRole('link',{name:'← 返回文章'});await expect(action).toBeVisible();const colors=await action.evaluate(element=>({text:getComputedStyle(element).color,page:getComputedStyle(document.body).backgroundColor}));expect(colors.text).not.toBe(colors.page);expect(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth)).toBe(false);await page.screenshot({path:`output/verification/pdf-reader-${info.project.name}-${mode}.png`,fullPage:true,animations:'disabled'});}
