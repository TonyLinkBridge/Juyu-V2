...
test('Q&A shortcut saves classification and order as draft metadata and preserves them on reload',async({page},info)=>{
 ...
 await page.getByLabel('问答排序',{exact:true}).fill('-1');await closeSettings(page);await expect(page.getByRole('alert')).toContainText('格式或长度');await settings(page);await expect(page.getByRole('button',{name:'立即保存',exact:true})).toBeDisabled();await closeSettings(page);await settings(page,'阅读范围与资料');
