...
test('adds category shortcut, changes roles, reorders and renders desktop/mobile light and dark',async({page},info)=>{
 ...
 await page.getByRole('button',{name:'添加入口',exact:true}).click();await settleConfirmation(page);
 ...
 await page.getByRole('button',{name:'上移',exact:true}).click();await settleConfirmation(page);await page.getByRole('button',{name:'上移',exact:true}).click();await settleConfirmation(page);
 await page.getByRole('button',{name:'保存导航',exact:true}).click();await settleConfirmation(page);
...
test('OPS choice grants no access, one role must remain and disabled categories stay explicit',async({page})=>{
 await mount(page);await entryButton(page,'运营知识').click();await settleConfirmation(page);
...
test('removes shortcut only and persists an intentionally empty or all-hidden configuration',async({page})=>{
 ...
 await page.getByRole('button',{name:'保存导航',exact:true}).click();await settleConfirmation(page);await expect(page.getByRole('status')).toContainText('全部快捷入口已隐藏');
 await page.getByRole('button',{name:'移除此入口',exact:true}).click();await settleConfirmation(page);await expect(page.getByText('没有快捷入口。保存空配置后，员工只保留固定系统入口。')).toBeVisible();
 await page.getByRole('button',{name:'保存导航',exact:true}).click();await settleConfirmation(page);await expect(page.getByRole('status')).toContainText('固定首页与退出入口仍保留');
...
test('uncertain acknowledgement locks exact payload and retries identical newly allocated identifiers',async({page})=>{
 ...
 await mount(page);await page.getByRole('button',{name:'添加入口',exact:true}).click();await settleConfirmation(page);await page.getByLabel('入口名称',{exact:true}).fill('稳定的新入口');
 await page.getByRole('button',{name:'保存导航',exact:true}).click();await settleConfirmation(page);await expect(page.getByRole('alert')).toContainText('无法确认');
 ...
 await page.getByRole('button',{name:'重试原提交',exact:true}).click();await settleConfirmation(page);await expect(page.getByRole('status')).toContainText('已保存');expect(writes).toHaveLength(2);expect(writes[0]).toEqual(writes[1]);
});
...
test('repeated conflicts preserve cumulative input backups and restoring rebases only the expected version',async({page})=>{
 ...
 for(const label of ['第一轮本地输入','第二轮本地输入']){
  await page.getByLabel('入口名称',{exact:true}).fill(label);await page.getByRole('button',{name:'保存导航',exact:true}).click();await settleConfirmation(page);await expect(page.getByRole('alert')).toContainText('已有新版本');
  await page.getByRole('button',{name:'保留备份并载入最新设置',exact:true}).click();await settleConfirmation(page);await expect(page.getByRole('status')).toContainText('已载入最新');
 }
 ...
 await page.getByRole('button',{name:'将备份 1 放回编辑区',exact:true}).click();await settleConfirmation(page);await expect(page.getByLabel('入口名称',{exact:true})).toHaveValue('第一轮本地输入');
 await page.getByRole('button',{name:'保存导航',exact:true}).click();await settleConfirmation(page);await expect(page.getByRole('status')).toContainText('已保存');expect(writes[2].expectedVersion).toBe(5);expect(writes[2].entries[0].label).toBe('第一轮本地输入');
...
test('unknown then conflict stays uncertain and failed reload preserves pending until a complete reload',async({page})=>{
 ...
 await mount(page);await page.getByLabel('入口名称',{exact:true}).fill('原提交名称');await page.getByRole('button',{name:'保存导航',exact:true}).click();await settleConfirmation(page);await page.getByRole('button',{name:'重试原提交',exact:true}).click();await settleConfirmation(page);
 await expect(page.getByRole('alert')).toContainText('仍无法确认此前是否保存成功');queueConfirmation(page,dialog=>dialog.accept());
 await page.getByRole('button',{name:'保留备份并载入最新设置',exact:true}).click();await settleConfirmation(page);await expect(page.getByRole('alert')).toContainText('无法完整载入');await expect(page.getByLabel('入口名称',{exact:true})).toHaveValue('原提交名称');await expect(page.getByLabel('入口名称',{exact:true})).toBeDisabled();
 await expect(page.getByLabel('配置备份 1',{exact:true})).toHaveCount(0);
 await page.getByRole('button',{name:'保留备份并载入最新设置',exact:true}).click();await settleConfirmation(page);await expect(page.getByLabel('入口名称',{exact:true})).toHaveValue('服务器当前入口');await expect(page.getByLabel('入口名称',{exact:true})).toBeEnabled();
...
test('dirty edits guard navigation and stay intact while switching between entries',async({page})=>{
 await mount(page);await page.getByLabel('入口名称',{exact:true}).fill('未保存入口');await entryButton(page,'运营知识').click();await settleConfirmation(page);await entryButton(page,'未保存入口').click();await settleConfirmation(page);await expect(page.getByLabel('入口名称',{exact:true})).toHaveValue('未保存入口');
 queueConfirmation(page,dialog=>dialog.dismiss());await page.getByRole('link',{name:'设置变更记录'}).click();await settleConfirmation(page);await expect(page).toHaveURL(/__navigation_settings_fixture$/);
 expect(await page.evaluate(()=>{const event=new Event('beforeunload',{cancelable:true});window.dispatchEvent(event);return event.defaultPrevented;})).toBe(true);
});
