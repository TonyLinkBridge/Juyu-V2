...
test('Help Centre home retains every JUYU entry inside the official Fumadocs home layout',async({page},info)=>{
 ...
 await expect(home.getByRole('link',{name:/Q&A 问答/})).toHaveAttribute('href','/help-centre/qa');
 await expect(home.getByRole('link',{name:/如何修改账户邮箱 已发布 · 正式资料/})).toHaveAttribute('href','/help-centre/articles/fumadocs-preview');
 ...
 expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
 await page.screenshot({path:`output/verification/fumadocs-home-${info.project.name}.png`,fullPage:true});
});
