...
test('record table supports search, column filter, sorting and card presentation',async({page})=>{
 const blocks=[{id:'lookup-table',type:'table',headers:['状态','说明'],rows:[['开放','先提交申请'],['关闭','暂停申请'],['开放','核对账户']],view:'grid',searchable:true,stickyHeader:true,stickyFirstColumn:true}];
 await page.route(url=>url.pathname==='/help-centre',route=>route.fulfill({contentType:'text/html',body:`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>${bundle.css}</style></head><body><div id="presentation"></div><script id="data" type="application/json">${JSON.stringify({pages:[{type:'document',id:'media-test',title:'资料表格',href:'/help-centre?article=media-test'}],requested:'media-test',article:{id:'media-test',title:'资料表格',revision:1,body:'正文',blocks}})}</script><script>${bundle.script}</script></body></html>`}));
 await page.goto('/help-centre');const table=page.locator('.reader-data-table');await expect(table).toBeVisible();await expect(table.locator('.sticky-header.sticky-first-column')).toHaveCount(1);
 await table.getByRole('searchbox',{name:'查找表格内容'}).fill('核对');await expect(table.getByRole('status')).toHaveText('显示 1 / 3 行');await expect(table.locator('.reader-data-table-grid').getByRole('cell',{name:'核对账户'})).toBeVisible();await expect(table.locator('.reader-data-table-grid').getByRole('cell',{name:'暂停申请'})).toHaveCount(0);
 await table.getByRole('searchbox',{name:'查找表格内容'}).fill('');await table.getByRole('combobox',{name:'筛选：状态'}).selectOption('关闭');await expect(table.getByRole('status')).toHaveText('显示 1 / 3 行');await expect(table.locator('.reader-data-table-grid').getByRole('cell',{name:'暂停申请'})).toBeVisible();await expect(table.locator('.reader-data-table-grid').getByRole('cell',{name:'先提交申请'})).toHaveCount(0);
 await table.getByRole('combobox',{name:'筛选：状态'}).selectOption('');const original=await table.locator('.reader-data-table-grid tbody tr').allTextContents();await table.getByRole('button',{name:'排序：说明'}).click();const sorted=await table.locator('.reader-data-table-grid tbody tr').allTextContents();expect(sorted).not.toEqual(original);await table.getByRole('button',{name:'排序：说明'}).click();expect(await table.locator('.reader-data-table-grid tbody tr').allTextContents()).toEqual([...sorted].reverse());
 blocks[0].view='cards';await page.reload();await expect(table.locator('.reader-data-table-cards')).toBeVisible();await expect(table.locator('.reader-data-table-grid')).toBeHidden();await expect(table.locator('.reader-data-table-cards article')).toHaveCount(3);
});
test('adjacent images open as a gallery and theme image follows the reader theme',async({page})=>{
 const dark='bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb';
 const blocks=[{id:'first',type:'image',assetId:asset,darkAssetId:dark,caption:'主图说明',alt:'主图'},{id:'second',type:'image',assetId:asset,caption:'补充图说明',alt:'补充图'}];
 await page.route(url=>url.pathname==='/help-centre',route=>route.fulfill({contentType:'text/html',body:`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>${bundle.css}</style></head><body><div id="presentation"></div><script id="data" type="application/json">${JSON.stringify({pages:[{type:'document',id:'media-test',title:'主题图片',href:'/help-centre?article=media-test'}],requested:'media-test',article:{id:'media-test',title:'主题图片',revision:1,body:'正文',blocks}})}</script><script>${bundle.script}</script></body></html>`}));
 await page.route('**/api/assets/**',route=>route.fulfill({path:'tests/fixtures/article-cover.png',contentType:'image/png'}));
 await page.goto('/help-centre');
 const gallery=page.locator('.reader-image-gallery.has-multiple');await expect(gallery).toHaveCount(1);
 await expect(gallery.getByAltText('主图')).toHaveAttribute('src',new RegExp(asset));
 await page.getByRole('radio',{name:'深色',exact:true}).check();await expect(gallery.getByAltText('主图')).toHaveAttribute('src',new RegExp(dark));
 await gallery.getByRole('button',{name:'放大图片：主图'}).click();const dialog=page.getByRole('dialog',{name:'图片放大查看'});await expect(dialog).toBeVisible();await expect(dialog.getByText('1 / 2')).toBeVisible();
 await dialog.getByRole('button',{name:'下一张'}).click();await expect(dialog.getByText('2 / 2')).toBeVisible();await page.keyboard.press('Escape');await expect(dialog).not.toBeVisible();await expect(gallery.getByRole('button',{name:'放大图片：补充图'})).toBeFocused();
});
test('native article images use the same zoom viewer without leaving the page',async({page})=>{
 const body='JUYU_BLOCKNOTE_V1\n'+JSON.stringify([{id:'native-image',type:'image',props:{url:'/api/assets/'+asset,name:'正文图片',caption:'正文截图'},children:[]}]);
 await page.route(url=>url.pathname==='/help-centre',route=>route.fulfill({contentType:'text/html',body:`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>${bundle.css}</style></head><body><div id="presentation"></div><script id="data" type="application/json">${JSON.stringify({pages:[{type:'document',id:'media-test',title:'正文图片',href:'/help-centre?article=media-test'}],requested:'media-test',article:{id:'media-test',title:'正文图片',revision:1,body,blocks:[]}})}</script><script>${bundle.script}</script></body></html>`}));
 await page.route('**/api/assets/**',route=>route.fulfill({path:'tests/fixtures/article-cover.png',contentType:'image/png'}));
 await page.goto('/help-centre');await page.getByRole('button',{name:'放大图片：正文图片'}).click();
 const dialog=page.getByRole('dialog',{name:'图片放大查看'});await expect(dialog).toBeVisible();await dialog.getByRole('button',{name:'关闭图片'}).click();await expect(dialog).not.toBeVisible();
});
