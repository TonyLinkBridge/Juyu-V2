...
test('unified body results show type revision context and retain canonical reader navigation',async({page},info)=>{
 ...
 const list=page.getByRole('list',{name:'搜索结果列表'});await expect(list.getByRole('link')).toHaveCount(4);await expect(list.locator('.search-result-snippet .search-match')).toHaveText(['二审','二审','二审','二审']);await expect(list).toContainText('OPS Internal');await expect(list).toContainText('Reference');await expect(list).toContainText('Q&A');await expect(list).toContainText('已发布');await expect(list).not.toContainText('正式版本 2');await expect(page.getByText('搜索你有权阅读的已发布资料：标题、标签和正文。')).toBeVisible();
