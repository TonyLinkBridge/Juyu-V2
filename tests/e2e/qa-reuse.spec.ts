...
test('R11 a server supplied independent answer opens immediately without a second request',async({page})=>{
 let reads=0;await page.route('**/api/qa/qa-local',r=>{reads++;return r.fulfill({json:answer});});
 await page.route('**/__qa_initial',r=>r.fulfill({contentType:'text/html',body:`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>${bundle.css}</style></head><body><div id="presentation"></div><script id="data" type="application/json">${JSON.stringify({qaAnswer:{id:answer.id,title:answer.title,revision:2,initial:answer}})}</script><script>${bundle.script.replaceAll('</script','<\\/script')}</script></body></html>`}));
 await page.goto('/__qa_initial');await expect(page.getByText(answer.body,{exact:true})).toBeVisible();await expect(page.getByText('正在读取标准答案…')).toHaveCount(0);expect(reads).toBe(0);
});
...
test('R11 focus revalidation discards revoked answers and retry obtains the current version',async({page})=>{
 ...
 await page.evaluate(()=>dispatchEvent(new Event('blur')));await expect(page.getByText(answer.body,{exact:true})).toBeVisible();await page.clock.runFor(30_001);await page.evaluate(()=>dispatchEvent(new Event('focus')));await expect(page.getByRole('alert')).toBeVisible();await expect(page.getByText(answer.body,{exact:true})).toHaveCount(0);await page.getByRole('button',{name:'重新读取'}).click();await expect(page.getByText('新版标准答案',{exact:true})).toBeVisible();expect(reads).toBe(3);
});
test('R11 returning after expiry keeps the answer visible while a refresh is unavailable',async({page})=>{
 await page.clock.install();let reads=0;await page.route('**/api/qa/qa-local',r=>{reads++;return reads===1?r.fulfill({json:answer}):r.fulfill({status:503});});await mount(page);await toggle(page).click();await expect(page.getByText(answer.body,{exact:true})).toBeVisible();await page.clock.runFor(30_001);await page.evaluate(()=>dispatchEvent(new Event('focus')));await expect(page.getByText(answer.body,{exact:true})).toBeVisible();await expect(page.getByText('答案已保留，最新状态暂时无法确认。')).toBeVisible();expect(reads).toBe(2);
});
