...
test('draft category choices persist with stable IDs and inherited access hints',async({page},info)=>{
 ...
 await mount(page,saved);await expect(page.getByRole('checkbox',{name:/已停用分类/})).toBeDisabled();
...
test('old disabled assignments can be retained or removed and rejected save keeps new choices',async({page})=>{
 const initial={...editorFixture,categoryOptions:defs,categoryIds:[id(4)]};await page.route('**/api/admin/editor/*',r=>r.fulfill({status:409,json:{error:'CONFLICT'}}));await mount(page,initial);
...
test('wrong category acknowledgment never shows saved and retries the identical submission',async({page})=>{
 const writes:unknown[]=[];await page.route('**/api/admin/editor/*',r=>{const input=r.request().postDataJSON();writes.push(input);return r.fulfill({json:{...editorFixture,...input,documentId:'editor-local',sequence:input.expectedSequence+1,categoryOptions:defs,categoryIds:[]}});});await mount(page,{...editorFixture,categoryOptions:defs,categoryIds:[]});await page.getByRole('checkbox',{name:/普通流程/}).check();
 await expect(page.getByRole('status')).toContainText('保存尚未确认',{timeout:8000});await expect(page.getByRole('status')).not.toContainText('所有修改已保存');await page.getByRole('button',{name:'重试保存'}).click();await expect.poll(()=>writes.length).toBe(2);expect(writes[1]).toEqual(writes[0]);
});
test('second review displays saved category scope and frozen editor preserves sent assignment',async({page})=>{
 ...
 await mount(page,article);await expect(page.getByRole('region',{name:'文章目录分类'})).toContainText('运营内部 / 异常处理');await expect(page.getByRole('checkbox',{name:/异常处理/})).toHaveCount(0);
});
