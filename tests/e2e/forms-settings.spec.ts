...
async function add(page:Page,field:FieldDefinition){await page.getByLabel('添加字段',{exact:true}).selectOption(field.id);await page.getByRole('button',{name:'添加到表单',exact:true}).click();}
...
test('creates disabled form using current fields, reorders local required/width and previews both themes',async({page},info)=>{
 ...
 await mount(page);await page.getByRole('button',{name:'新建表单',exact:true}).click();
 ...
 await binding(page,date).getByRole('button',{name:'上移',exact:true}).click();
...
test('existing bindings retain snapshots until explicit refresh and local required stays independent',async({page})=>{
 ...
 await binding(page,team).getByRole('button',{name:'更新为最新字段',exact:true}).click();
...
test('disabled global fields can be retained while disabling a form after explicit confirmation',async({page})=>{
 ...
 await page.getByRole('button',{name:/入职申请/}).click();
 await expect(page.getByRole('button',{name:'更新为最新字段',exact:true})).toHaveCount(0);
 await page.getByLabel('启用表单',{exact:true}).uncheck();
 page.once('dialog',async dialog=>{expect(dialog.message()).toContain('立即生效');await dialog.dismiss();});
 await page.getByRole('button',{name:'保存表单',exact:true}).click();expect(writes).toHaveLength(0);
 page.once('dialog',dialog=>dialog.accept());await page.getByRole('button',{name:'保存表单',exact:true}).click();
 await expect(page.getByRole('status')).toContainText('停用状态');
...
test('unknown then conflict remains uncertain and explicit reload restores current configuration',async({page})=>{
 ...
 page.once('dialog',async dialog=>{expect(dialog.message()).toContain('不会撤销');await dialog.accept();});
 await page.getByRole('button',{name:'载入最新设置（替换当前输入）',exact:true}).click();
...
test('empty configuration, unavailable and denied remain distinct; empty field list cannot be saved',async({page})=>{
 ...
 await page.getByRole('button',{name:'新建表单',exact:true}).click();
 await page.getByLabel('表单标题',{exact:true}).fill('无字段');
 await page.getByRole('button',{name:'保存表单',exact:true}).click();
 await expect(page.getByRole('alert')).toContainText('1–20');expect(writes).toBe(0);
 page.once('dialog',dialog=>dialog.accept());await page.unrouteAll();await mount(page,{initial:[form],definitions,state:'unavailable'});
...
test('enabling a brand new form requires confirmation and dismissing sends no write',async({page})=>{
 ...
 page.once('dialog',async dialog=>{expect(dialog.message()).toContain('立即开放的新表单');expect(dialog.message()).toContain('运营和管理员');expect(dialog.message()).toContain('立即生效');await dialog.dismiss();});
 await page.getByRole('button',{name:'保存表单',exact:true}).click();
