...
test('member identity accepts only current verified provider role and matching primary email', () => {
  ...
  for(const role of ['support','ops']) assert.equal(verifiedMember(company,{...user,publicMetadata:{role}})?.role,role);
...
test('member API protects writes with exact origin and maps failures without details',async()=>{
 ...
 for(const [error,status] of [['MEMBER_PENDING',409],['FORBIDDEN: secret',403],['AUTH_NOT_CONFIGURED',503],['provider secret',503],['INVALID_INPUT',400]] as const){
